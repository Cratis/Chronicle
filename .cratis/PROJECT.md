...
- **`.cratis/ai.json`** records the subscription — `cratis/documentation` for docs-writing guidance, `cratis/engineering/csharp` and `cratis/engineering/react` for the maintainer conventions of the kernel (C#) and the Workbench (React), and `cratis/application/csharp` + `cratis/application/react` because this repository also carries the application surfaces built on the stack.
