...
Two rules follow from how the artifacts are read, and both fail quietly if you break them:

- **A conversion helper does not belong on a `[ReadModel]`.** The proxy generator emits a query proxy for any
  static method whose return shape is a supported query shape, and does not look at accessibility — so a private
  `ToWhatever` helper becomes a public HTTP endpoint with a generated client. Put helpers in a separate
  `*Converters` class beside the read model.
- **A read model must not share a name with a `Contracts.*` type it references.** The generated TypeScript imports
  the one and declares the other in the same file. `IdentityDetails` is named that way for this reason.

### Current state of the conversion

Generation from Core into Contracts **runs by default**. `DisableGrpcContractGeneration` defaults to `false` in
`Core.csproj`, so an ordinary build of Core regenerates the contracts, the service implementations under
`../Grpc`, and the registrations in `../Server`:

```bash
dotnet build Source/Kernel/Core/Core.csproj -c Debug -p:CratisProxiesOutputPath=
```

`CratisProxiesOutputPath=` there only skips the *TypeScript proxy* generator, which is a separate switch
(`DisableProxyGenerator`, also defaulting to run). The two were deliberately split so that enabling one cannot
silently re-run the other.

So a Core change reaches the wire without a special flag, and the checked-in contracts for derived services
cannot quietly drift from Core — the next Core build rewrites them. **Add a field by changing the Core
artifact and building**, then commit what generation produced; never hand-edit the `<auto-generated />` files
under `Source/Kernel/Contracts`.

`CopyLocalLockFileAssemblies=true`, which `Core.csproj` sets, is required: the generator resolves referenced
assemblies from the output directory, and a library project does not copy its NuGet runtime assets there, so
`Orleans.Serialization` is missing and generation aborts.

`NonDerivedGrpcServices` in `Core.csproj` lists the services the generator leaves entirely alone — contract *and*
implementation. It holds `Observers`, whose `Contracts/Observation/IObservers.cs` carries five methods Core has no
artifacts for (`GetObserverInformation`, `GetConnectedClientsForObserver`, `WaitForCompletion`,
`GetReplayableObserversForEventTypes`, `ClearObserverQuarantine`) and names its other rpcs differently from what
the artifacts would produce. Emptying that list is what converting Observation delivers, and it is the remaining
work — not the target gating this once was.
