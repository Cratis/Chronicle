...
`DisableGrpcContractGeneration` defaults to `true` there, because most of the Web API surface still lives in
`Source/Clients/Api` rather than Core. See `PLAN.md`.
...
the artifacts would produce. Emptying that list is what converting Observation delivers.
...
dotnet build Source/Kernel/Core/Core.csproj -c Debug \
  -p:DisableGrpcContractGeneration=false -p:CratisProxiesOutputPath=
```
