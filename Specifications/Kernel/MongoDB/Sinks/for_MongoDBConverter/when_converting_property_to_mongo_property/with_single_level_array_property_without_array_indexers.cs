...
namespace Aksio.Cratis.Kernel.MongoDB.Sinks.for_MongoDBConverter.when_converting_property_to_mongo_property;

public class with_single_level_array_property_without_array_indexers : given.a_mongodb_converter
{
    MongoDBProperty result;

    void Because() => result = converter.ToMongoDBProperty(new PropertyPath("[ArrayProperty]"), ArrayIndexers.NoIndexers);
