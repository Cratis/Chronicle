...
public class when_catching_up_with_two_events_to_catch_up_with : given.a_catch_up_worker_with_two_pending_events
{
    ...
    [Fact] void should_call_on_next_for_first_event() => subscriber.Verify(_ => _.OnNext(Is<IEnumerable<AppendedEvent>>(m => m.First() == first_appended_event), new(IsAny<EventObservationState>(), subscriber_args)), Once);
    [Fact] void should_call_on_next_for_second_event() => subscriber.Verify(_ => _.OnNext(Is<IEnumerable<AppendedEvent>>(m => m.First() == second_appended_event), new(IsAny<EventObservationState>(), subscriber_args)), Once);
