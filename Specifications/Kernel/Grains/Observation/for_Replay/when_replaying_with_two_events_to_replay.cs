...
public class when_replaying_with_two_events_to_replay : given.a_replay_with_two_pending_events
{
    ...
    void Establish()
    {
        ...
        subscriber.Setup(_ => _.OnNext(IsAny<AppendedEvent>(), IsAny<ObserverSubscriberContext>())).Callback((AppendedEvent @event, ObserverSubscriberContext _) => events_replayed.Add(@event));
    }
