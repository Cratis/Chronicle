...
public class and_subscriber_is_disconnected : given.an_observer_worker
{
    ...
    void Establish()
    {
        ...
        subscriber.Setup(_ => _.OnNext(IsAny<IEnumerable<AppendedEvent>>(), IsAny<ObserverSubscriberContext>())).Returns((IEnumerable<AppendedEvent> e) => Task.FromResult(ObserverSubscriberResult.Disconnected));
    }
