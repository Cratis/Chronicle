...
public class and_two_failed_partitions : given.a_connected_observer_with_event_types_and_reminder
{
    ...
    [Fact] void should_subscribe_to_sequences_stream_for_both_partitions() => sequence_stream.Verify(_ => _.SubscribeAsync(IsAny<IAsyncObserver<AppendedEvent>>(), IsAny<StreamSequenceToken>(), IsAny<StreamFilterPredicate>(), IsAny<object>()), Exactly(2));
