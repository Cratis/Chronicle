...
public class when_subscribing_with_two_tenants_first_time_for_connection : given.two_tenants_and_two_event_types
{
    ...
    [Fact] void should_hold_observer_with_correct_microservice_in_key_associated_with_connection_id_for_first_tenant_subscriber() => state_on_write.GetObserversForConnectionId(connection_id).First().ObserverKey.MicroserviceId.ShouldEqual(execution_context.MicroserviceId);
    [Fact] void should_hold_observer_with_correct_tenant_in_key_associated_with_connection_id_for_first_tenant_subscriber() => state_on_write.GetObserversForConnectionId(connection_id).First().ObserverKey.TenantId.ShouldEqual(first_tenant);
    [Fact] void should_hold_observer_with_correct_microservice_in_key_associated_with_connection_id_for_second_tenant_subscriber() => state_on_write.GetObserversForConnectionId(connection_id).ToArray()[1].ObserverKey.MicroserviceId.ShouldEqual(execution_context.MicroserviceId);
    [Fact] void should_hold_observer_with_correct_tenant_in_key_associated_with_connection_id_for_second_tenant_subscriber() => state_on_write.GetObserversForConnectionId(connection_id).ToArray()[1].ObserverKey.TenantId.ShouldEqual(second_tenant);
