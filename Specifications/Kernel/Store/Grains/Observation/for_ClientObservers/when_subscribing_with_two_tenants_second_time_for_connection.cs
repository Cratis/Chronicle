...
{
    const string name = "MyObserver";

    async Task Establish() => await observers.Subscribe(name, observer_id, event_sequence_id, event_types);

    async Task Because() => await observers.Subscribe(name, observer_id, event_sequence_id, event_types);
