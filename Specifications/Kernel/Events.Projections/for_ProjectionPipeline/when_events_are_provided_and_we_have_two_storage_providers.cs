...
namespace Cratis.Events.Projections.for_ProjectionPipeline
{
    public class when_events_are_provided_and_we_have_two_storage_providers : given.a_pipeline
    {
        ...
        void Establish()
        {
            ...
            pipeline.Start();
            @event = new Event(0, "8ccc9ac5-50bc-4791-97ca-8a03bc4a6ccf", DateTimeOffset.UtcNow, "14dea6ef-bc08-4fb3-86a3-21dedee157fd", new{});
        }

        void Because() => subject.OnNext(@event);

        [Fact] void should_call_projection_for_first_storage() => projection.Verify(_ => _.OnNext(@event, first_storage.Object), Once());
        [Fact] void should_call_projection_for_second_storage() => projection.Verify(_ => _.OnNext(@event, second_storage.Object), Once());
    }
}
