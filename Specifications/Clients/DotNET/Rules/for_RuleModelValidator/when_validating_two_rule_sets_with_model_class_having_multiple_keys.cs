...
public class when_validating_two_rule_sets_with_model_class_having_multiple_keys : given.two_rule_sets
{
    ...
    ModelClassWithMultipleKeys model;
    Exception result;

    void Establish()
    {
        model_metadata_provider = new();
        model_metadata = new(ModelMetadataIdentity.ForType(typeof(ModelClassWithMultipleKeys)));
        model = new ModelClassWithMultipleKeys
        {
            Id = key,
            SecondId = key
        };
