...
using context = Cratis.Chronicle.Integration.Cli.for_Events.when_getting_events.context;
...
public class when_getting_events(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because() => Result = await RunCliAsync("events", "get", "--event-store", "system");
    }
