...
using context = Cratis.Chronicle.Integration.Cli.for_Users.when_listing_users.context;

namespace Cratis.Chronicle.Integration.Cli.for_Users;

[Collection(ChronicleCollection.Name)]
public class when_listing_users(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        public CliCommandResult Result = null!;

        async Task Because() => Result = await RunCliAsync("users", "list");
    }
