...
using context = Cratis.Chronicle.Integration.Cli.for_Namespaces.when_listing_namespaces.context;

namespace Cratis.Chronicle.Integration.Cli.for_Namespaces;

[Collection(ChronicleCollection.Name)]
public class when_listing_namespaces(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        public CliCommandResult Result = null!;

        async Task Because() => Result = await RunCliAsync("namespaces", "list", "--event-store", "system");
    }
