...
using context = Cratis.Chronicle.Integration.Cli.for_Projections.when_listing_projections.context;

namespace Cratis.Chronicle.Integration.Cli.for_Projections;

[Collection(ChronicleCollection.Name)]
public class when_listing_projections(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        public CliCommandResult Result = null!;

        async Task Because() => Result = await RunCliAsync("projections", "list", "--event-store", "system");
    }
