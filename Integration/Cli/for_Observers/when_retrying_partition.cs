...
using context = Cratis.Chronicle.Integration.Cli.for_Observers.when_retrying_partition.context;
...
public class when_retrying_partition(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because()
        {
            ...
            Result = await RunCliAsync("observers", "retry-partition", ObserverId, "test-partition-key", "--event-store", "system");
        }
    }
