...
using context = Cratis.Chronicle.Integration.Cli.for_Observers.when_replaying_partition.context;

namespace Cratis.Chronicle.Integration.Cli.for_Observers;

[Collection(ChronicleCollection.Name)]
public class when_replaying_partition(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because()
        {
            ...
            Result = await RunCliAsync("observers", "replay-partition", ObserverId, "test-partition-key", "--event-store", "system");
        }
    }

    [Fact] void should_return_success_exit_code() => Context.Result.ExitCode.ShouldEqual(ExitCodes.Success);

    [Fact] void should_contain_replay_started_message() => Context.Result.StandardOutput.ShouldContain("Replay started for partition");
