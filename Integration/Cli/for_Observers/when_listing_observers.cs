...
using context = Cratis.Chronicle.Integration.Cli.for_Observers.when_listing_observers.context;
...
public class when_listing_observers(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because() => Result = await RunCliAsync("observers", "list", "--event-store", "system");
    }
