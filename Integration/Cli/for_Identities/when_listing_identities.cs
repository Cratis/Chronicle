...
using context = Cratis.Chronicle.Integration.Cli.for_Identities.when_listing_identities.context;

namespace Cratis.Chronicle.Integration.Cli.for_Identities;
...
public class when_listing_identities(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because() => Result = await RunCliAsync("identities", "list", "--event-store", "system");
    }
