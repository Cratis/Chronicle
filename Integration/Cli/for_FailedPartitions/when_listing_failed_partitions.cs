...
using context = Cratis.Chronicle.Integration.Cli.for_FailedPartitions.when_listing_failed_partitions.context;
...
public class when_listing_failed_partitions(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because() => Result = await RunCliAsync("failed-partitions", "list", "--event-store", "system");
    }
