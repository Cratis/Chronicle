...
using context = Cratis.Chronicle.Integration.Cli.for_EventTypes.when_listing_event_types.context;

namespace Cratis.Chronicle.Integration.Cli.for_EventTypes;

[Collection(ChronicleCollection.Name)]
public class when_listing_event_types(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        public CliCommandResult Result = null!;

        async Task Because() => Result = await RunCliAsync("event-types", "list", "--event-store", "system");
    }
