...
using context = Cratis.Chronicle.Integration.Cli.for_Recommendations.when_listing_recommendations.context;
...
public class when_listing_recommendations(context context) : CliGiven<context>(context)
{
    public class context : given.a_connected_cli
    {
        ...
        async Task Because() => Result = await RunCliAsync("recommendations", "list", "--event-store", "system");
    }
