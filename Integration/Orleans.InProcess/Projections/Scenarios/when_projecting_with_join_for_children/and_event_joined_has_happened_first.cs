...
using context = Cratis.Chronicle.Integration.Orleans.InProcess.Projections.Scenarios.when_projecting_with_join_for_children.and_event_joined_has_happened_first.context;
...
public class and_event_joined_has_happened_first(context context) : Given<context>(context)
{
    ...
    public class context(GlobalFixture globalFixture) : given.a_projection_and_events_appended_to_it<ProjectionWithJoinOnChildren, User>(globalFixture)
    {
    ...
    [Fact] void should_have_group_id() => Context.Result.Groups.First().GroupId.ShouldEqual(Context.GroupId);
    [Fact] void should_have_group_name() => Context.Result.Groups.First().GroupName.ShouldEqual(GroupName);
}
