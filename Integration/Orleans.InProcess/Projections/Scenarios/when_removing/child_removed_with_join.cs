...
{
    public class context(ChronicleMongoDBFixture chronicleMongoDbFixture) : given.a_projection_and_events_appended_to_it<UserProjectionWithRemovedWithJoin, User>(chronicleMongoDbFixture)
    {
        ...
        async Task Because()
        {
            var result = await ChronicleMongoDbFixture.ReadModels.Database.GetCollection<User>().FindAsync(_ => true);
