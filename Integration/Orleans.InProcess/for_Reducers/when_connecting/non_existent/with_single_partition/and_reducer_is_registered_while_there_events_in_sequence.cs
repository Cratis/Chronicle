...
using context = Cratis.Chronicle.Integration.Orleans.InProcess.for_Reducers.when_connecting.non_existent.with_single_partition.and_reducer_is_registered_while_there_events_in_sequence.context;
using ObserverRunningState = Cratis.Chronicle.Concepts.Observation.ObserverRunningState;

namespace Cratis.Chronicle.Integration.Orleans.InProcess.for_Reducers.when_connecting.non_existent.with_single_partition;

[Collection(GlobalCollection.Name)]
public class and_reducer_is_registered_while_there_events_in_sequence(context context) : Given<context>(context)
{
    public class context(GlobalFixture globalFixture, ITestOutputHelper testOutputHelper) : given.a_disconnected_reducer_observing_an_event(globalFixture)
    {
