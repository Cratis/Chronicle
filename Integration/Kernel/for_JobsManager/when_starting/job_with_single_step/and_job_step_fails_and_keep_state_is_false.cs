...
using Cratis.Monads;
using context = Cratis.Chronicle.Integration.Specifications.for_JobsManager.when_starting.job_with_single_step.and_job_step_fails_and_keep_state_is_false.context;
...
public class and_job_step_fails_and_keep_state_is_false(context context) : Given<context>(context)
{
    public class context(ChronicleFixture chronicleFixture) : given.a_jobs_manager(chronicleFixture)
    {
        ...
        async Task Because()
        {
            ...
            Job = await EventStore.Jobs.WaitTillJobProgressCompleted(JobId.Value);
