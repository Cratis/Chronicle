...
public class when_getting_all_event_types(context context) : Given<context>(context)
{
    public class context(ChronicleOutOfProcessFixtureWithLocalImage fixture) : given.an_http_client(fixture)
    {
        ...
        public IEnumerable<EventType> Data;

        async Task Establish()
        {
            await Client.ExecuteCommand("/api/event-store/test-store/types", new RegisterEventTypes(
            [
        ...
        async Task Because()
        {
            Result = await Client.ExecuteQuery<IEnumerable<EventType>>("/api/event-store/test-store/types");
            Data = Result.Data as IEnumerable<EventType>;
        }
    }
    ...
    [Fact]
    void should_include_first_event_type() => Context.Data.ShouldContain(_ => _.Id == "26f57829-c3a0-45ca-b1fc-2e05e6e54b8e");

    [Fact]
    void should_include_second_event_type() => Context.Data.ShouldContain(_ => _.Id == "8f7b4a3c-d2e1-4f9a-b8c7-1d6e3f2a4b5c");
}
