...
public class and_reactors_coordinate_tenant_outbox_forwarding(context context) : Given<context>(context)
{
    public class context(ChronicleFixture chronicleInProcessFixture) : Specification(chronicleInProcessFixture)
    {
        ...
        async Task Because()
        {
            ...
            await Task.WhenAll(
                connectionServices.Services.Namespaces.EnsureNamespace(new EnsureNamespaceRequest { EventStore = SourceEventStoreName, Namespace = TenantANamespace }).EnsureSuccess(),
                connectionServices.Services.Namespaces.EnsureNamespace(new EnsureNamespaceRequest { EventStore = SourceEventStoreName, Namespace = TenantBNamespace }).EnsureSuccess(),
                connectionServices.Services.Namespaces.EnsureNamespace(new EnsureNamespaceRequest { EventStore = TargetEventStoreName, Namespace = TenantANamespace }).EnsureSuccess(),
                connectionServices.Services.Namespaces.EnsureNamespace(new EnsureNamespaceRequest { EventStore = TargetEventStoreName, Namespace = TenantBNamespace }).EnsureSuccess());
            ...
            var reactorActivationTimeout = TimeSpanFactory.FromSeconds(60);
            await targetReactorA.WaitTillSubscribed(reactorActivationTimeout);
            await targetReactorB.WaitTillSubscribed(reactorActivationTimeout);
            await targetReactorA.WaitTillActive(reactorActivationTimeout);
