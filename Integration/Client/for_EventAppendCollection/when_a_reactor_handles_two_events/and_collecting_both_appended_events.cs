...
using context = Cratis.Chronicle.Integration.for_EventAppendCollection.when_a_reactor_handles_two_events.and_collecting_both_appended_events.context;

namespace Cratis.Chronicle.Integration.for_EventAppendCollection.when_a_reactor_handles_two_events;

[Collection(ChronicleCollection.Name)]
public class and_collecting_both_appended_events(context context) : Given<context>(context)
{
    public class context(ChronicleFixture fixture) : given.an_event_append_collection_scope(fixture)
    {
        async Task Because()
        {
