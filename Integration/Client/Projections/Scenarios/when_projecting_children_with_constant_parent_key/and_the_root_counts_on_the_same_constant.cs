...
using context = Cratis.Chronicle.Integration.Projections.Scenarios.when_projecting_children_with_constant_parent_key.and_the_root_counts_on_the_same_constant.context;

namespace Cratis.Chronicle.Integration.Projections.Scenarios.when_projecting_children_with_constant_parent_key;

[Collection(ChronicleCollection.Name)]
public class and_the_root_counts_on_the_same_constant(context context) : Given<context>(context)
{
    public class context(ChronicleFixture chronicleFixture) : given.a_projection_and_events_appended_to_it<RootAndChildrenWithConstantKeyProjection, CountsReadModel>(chronicleFixture)
    {
        ...
        void Establish()
        {
            ReadModelId = RootAndChildrenWithConstantKeyProjection.ConstantKeyValue;
    ...
    [Fact] void should_materialize_the_document() => Context.Result.ShouldNotBeNull();
    [Fact] void should_count_every_registration_at_the_root() => Context.Result.Registrations.ShouldEqual(3);
