...
public class UserRegisteredReducer : IReducerFor<UserReadModel>
{
    ...
    public Task<UserReadModel?> OnUserRegisteredV2(UserRegisteredV2 evt, UserReadModel? input, EventContext ctx)
    {
        ...
        input ??= new UserReadModel(evt.FirstName, evt.LastName);
        return Task.FromResult<UserReadModel?>(input with { FirstName = evt.FirstName, LastName = evt.LastName });
    }
