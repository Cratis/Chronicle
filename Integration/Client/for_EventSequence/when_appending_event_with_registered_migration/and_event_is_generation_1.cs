...
public class and_event_is_generation_1(context context) : Given<context>(context)
{
    ...
    [Fact] Task should_have_split_first_name_into_generation_2_content() => Context.ShouldHaveAppendedEvent<PersonRegistered>(0, Context.EventSourceId.Value, e => e.FirstName.ShouldEqual("John"));
    [Fact] Task should_have_split_last_name_into_generation_2_content() => Context.ShouldHaveAppendedEvent<PersonRegistered>(0, Context.EventSourceId.Value, e => e.LastName.ShouldEqual("Doe"));
}
