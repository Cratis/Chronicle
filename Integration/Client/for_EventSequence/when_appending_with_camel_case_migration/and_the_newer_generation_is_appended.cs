...
using context = Cratis.Chronicle.Integration.for_EventSequence.when_appending_with_camel_case_migration.and_the_newer_generation_is_appended.context;
...
public class and_the_newer_generation_is_appended(context context) : Given<context>(context)
{
    public class context(ChronicleFixture fixture) : given.a_camel_case_client(fixture)
    {
        async Task Because()
        {
            Result = await _store.EventLog.Append("contact", new ContactRecorded(new ContactDetails("person@example.com", "work"), "active"));
    ...
    [Fact] void should_migrate_the_nested_property() => Context.GenerationOne["contact"]!["emailAddress"]!.GetValue<string>().ShouldEqual("person@example.com");
    [Fact] void should_preserve_the_unmapped_sibling() => Context.GenerationOne["contact"]!["label"]!.GetValue<string>().ShouldEqual("work");
