...
public class when_a_seeded_batch_is_rejected(context context) : Given<context>(context)
{
    ...
    /// <summary>
    /// The rejected run must NOT surface as an exception. Seeding happens inside <c language="csharp">RegisterAll</c>, which the
    /// client runs from <c language="csharp">OnConnected</c>; a failing handler there rolls the connection back to disconnected,
