...
using context = Cratis.Chronicle.Integration.Specifications.Projections.Scenarios.when_projecting_with_functions.incrementing_with_multiple_events.context;

namespace Cratis.Chronicle.Integration.Specifications.Projections.Scenarios.when_projecting_with_functions;

[Collection(ChronicleCollection.Name)]
public class incrementing_with_multiple_events(context context) : Given<context>(context)
{
    public class context(ChronicleFixture chronicleFixture) : given.a_projection_and_events_appended_to_it<IncrementingProjection, ReadModel>(chronicleFixture)
    {
    ...
    [Fact] void should_increment_once_per_event() => Context.Result.IntValue.ShouldEqual(3);
