...
public class and_collecting_both_appended_events(context context) : Given<context>(context)
{
    ...
    [Fact] void should_have_the_first_event() => Context.AppendedEventsCollector.All[0].Event.ShouldBeOfExactType<AnEventHappened>();
    [Fact] void should_have_the_second_event() => Context.AppendedEventsCollector.All[1].Event.ShouldBeOfExactType<AnotherEventHappened>();
    [Fact] void should_both_be_successful() => Context.AppendedEventsCollector.All.All(e => e.IsSuccess).ShouldBeTrue();
    [Fact] void should_both_be_for_the_correct_event_source() => Context.AppendedEventsCollector.All.All(e => e.EventSourceId == Context.EventSourceId).ShouldBeTrue();
}
