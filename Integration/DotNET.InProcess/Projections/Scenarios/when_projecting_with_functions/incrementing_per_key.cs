...
using context = Cratis.Chronicle.InProcess.Integration.Projections.Scenarios.when_projecting_with_functions.incrementing_per_key.context;

namespace Cratis.Chronicle.InProcess.Integration.Projections.Scenarios.when_projecting_with_functions;

[Collection(ChronicleCollection.Name)]
public class incrementing_per_key(context context) : Given<context>(context)
{
    public class context(ChronicleInProcessFixture chronicleInProcessFixture) : given.a_projection_and_events_appended_to_it<IncrementingProjection, ReadModel>(chronicleInProcessFixture)
    {
    ...
    [Fact] void should_increment_correctly_for_second_key() => Context.SecondResult.IntValue.ShouldEqual(2);
}
