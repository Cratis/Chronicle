...
using context = Cratis.Chronicle.InProcess.Integration.for_Reactors.when_connecting.non_existent.with_multiple_partitions.and_reactor_is_registered_while_there_are_events_in_sequence.context;

namespace Cratis.Chronicle.InProcess.Integration.for_Reactors.when_connecting.non_existent.with_multiple_partitions;
...
public class and_reactor_is_registered_while_there_are_events_in_sequence(context context) : Given<context>(context)
{
    ...
    [Fact]
    void should_process_all_events() => (Context.Reactor.HandledEvents - Context.HandledEventsBefore).ShouldBeGreaterThanOrEqual(Context.Events.Count);
}
