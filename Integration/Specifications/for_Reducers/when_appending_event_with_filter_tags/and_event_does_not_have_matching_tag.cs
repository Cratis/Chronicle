...
using context = Cratis.Chronicle.Integration.Specifications.for_Reducers.when_appending_event_with_filter_tags.and_event_does_not_have_matching_tag.context;

namespace Cratis.Chronicle.Integration.Specifications.for_Reducers.when_appending_event_with_filter_tags;

[Collection(ChronicleCollection.Name)]
public class and_event_does_not_have_matching_tag(context context) : Given<context>(context)
{
    public class context(ChronicleFixture chronicleInProcessFixture) : Specification<ChronicleFixture>(chronicleInProcessFixture)
    {
        ...
        async Task Because()
        {
            ...
            // Append an event with the matching tag — should be handled
            await EventStore.EventLog.Append(EventSourceId, MatchingEvent, tags: ["important"]);
