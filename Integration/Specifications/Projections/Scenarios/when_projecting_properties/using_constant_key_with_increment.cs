...
using context = Cratis.Chronicle.Integration.Specifications.Projections.Scenarios.when_projecting_properties.using_constant_key_with_increment.context;

namespace Cratis.Chronicle.Integration.Specifications.Projections.Scenarios.when_projecting_properties;

[Collection(ChronicleCollection.Name)]
public class using_constant_key_with_increment(context context) : Given<context>(context)
{
    public class context(ChronicleInProcessFixture chronicleInProcessFixture) : given.a_projection_and_events_appended_to_it<IncrementWithConstantKeyProjection, CounterReadModel>(chronicleInProcessFixture)
    {
        public override IEnumerable<Type> EventTypes => [typeof(EmptyEvent)];

        void Establish()
        {
            ReadModelId = IncrementWithConstantKeyProjection.ConstantKeyValue;
    ...
    [Fact] void should_increment_for_all_events_regardless_of_event_source() => Context.Result.Count.ShouldEqual(3);
