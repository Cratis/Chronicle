...
using context = Cratis.Chronicle.Integration.Specifications.Projections.Scenarios.when_projecting_properties.using_constant_key.context;

namespace Cratis.Chronicle.Integration.Specifications.Projections.Scenarios.when_projecting_properties;

[Collection(ChronicleCollection.Name)]
public class using_constant_key(context context) : Given<context>(context)
{
    public class context(ChronicleInProcessFixture chronicleInProcessFixture) : given.a_projection_and_events_appended_to_it<ConstantKeyProjection, ReadModel>(chronicleInProcessFixture)
    {
        ...
        void Establish()
        {
            ReadModelId = ConstantKeyProjection.ConstantKeyValue;
            ExpectedStringValue = Guid.NewGuid().ToString();
