...
using context = Cratis.Chronicle.Integration.Specifications.for_Reactors.when_connecting.existing.with_single_partition.and_reactor_has_observed_events_previously_but_is_now_behind.context;

namespace Cratis.Chronicle.Integration.Specifications.for_Reactors.when_connecting.existing.with_single_partition;
...
public class and_reactor_has_observed_events_previously_but_is_now_behind(context context) : Given<context>(context)
{
    ...
    [Fact]
    void should_process_all_events() => (Context.Reactor.HandledEvents - Context.HandledEventsBefore).ShouldEqual(Context.FirstEvents.Count + Context.CatchupEvents.Count);
}
