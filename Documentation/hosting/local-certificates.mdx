...
This guide explains how to set up your own TLS certificate for local Chronicle development. In development, Chronicle already serves its single port over TLS with an auto-generated self-signed certificate — so you only need this when you want to supply a specific certificate, for example to test with a trusted certificate or to simulate a production setup.
