...
Discovery matches on the parameter types, so a helper you extract out of a reducer method keeps the same shape the reducer method has and matches the same event type. When that happens, Chronicle picks one method per event type, in this order:

1. **A public method beats a non-public one.** Reducer methods are public by convention; anything else is an implementation detail, so a private helper never displaces the method that calls it.
2. **The richest signature wins.** Between two methods of the same accessibility, the one taking the most parameters asked for the most context — so the overload that also takes the `EventContext` is preferred.
