...
Kotlin and Java expose the `isActive` toggle on `@Reducer`, and Elixir the `active: false` option on `use Chronicle.Reducers.Reducer` (Option 1 below). The read-model-level `[Passive]` marker (Option 2) is C#-only. TypeScript's `reducer()` decorator has neither.
:::
