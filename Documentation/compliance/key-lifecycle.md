...
- **A provisioning already in flight when the fence lands.** Reading the fence and writing the key are two operations, not one, in every backend except the in-memory store. A provisioning that read "not erased" a moment before the fence was recorded can still persist its key a moment after. Recording the fence everywhere before destroying anything narrows that window to the width of a single write, and the erasure destroys whatever it finds — but a key minted in the gap between the destruction and the last write survives until the erasure is repeated. Erase again after forwarding and appends for the subject have stopped.
