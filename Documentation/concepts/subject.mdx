...
## Reading the subject back

The subject travels with the event. Whatever an event was appended with is what you get back on
`EventContext.Subject` — both when reading an event sequence and when an observer such as a reactor
or reducer is notified. When no explicit subject was supplied, that value is the `EventSourceId`,
exactly as on append.

This matters when an observer forwards an event onward: forwarding `context.Subject` keeps the
original compliance identity, so the encryption key, erasure target and cross-store propagation stay
with the person the event is about rather than with the stream it happened to be written to.
...
That one operation covers one event store and one namespace. A subject whose events are
forwarded to another event store by a subscription holds a key in that store too, and erasing
them takes one delete per event store and namespace — see
