...
description: "Evolve an event type through generations with upcast and downcast migrations, and what each stored generation holds."
---
...
- The generation an event was appended as is stored as written, so the original content is never overwritten. The other generations hold only what the migrations produce: the example above splits a name on spaces, so for "Mary Ann Smith" the upcast generation 2 keeps "Mary" and "Ann" and drops "Smith". Write migrations that keep every part a consumer needs.
