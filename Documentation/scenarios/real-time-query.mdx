---
title: Get read models
description: Retrieve read model instances — one, all, or a page — and observe them change live, picking the right consistency for each read.
---

**Goal:** your events project into a read model — now you need it back out. One book, the whole catalog, a page for a grid, or a live stream that updates the UI as events arrive. `IReadModels` (on `IEventStore.ReadModels`) covers all of these; the choice is between **eventually consistent** reads served from the materialized sink — the default — and **strongly consistent** reads of a passive read model, computed from the event log on every call.

:::note[Client coverage]
Getting a single instance (below) is real in every client. Kotlin and Java currently only support that single-instance lookup — no bulk "get all", materialized paging, or live/observable read model API exists in the Kotlin SDK yet. Elixir supports both "get all" (`Chronicle.all/1`) and materialized paging (`Chronicle.ReadModels.query/2`, with `page`/`page_size`) — see [Designing read models](../concepts/designing-read-models) — but has no live/observable query API. TypeScript is the only client with a live, observable read model query (`readModels.watch()`) in addition to the single/bulk/paged reads.
:::

## Get one instance

`GetInstanceById` returns the instance for one key. For a materialized read model — the default — it reads the stored instance from the sink, so it is eventually consistent. For a `[Passive]` read model it replays the key's events through the projection or reducer on demand, reflecting everything appended up to this moment:

<ChronicleClientTabs snippet="scenarios/real-time-query/get-one" />

`ReadModelKey` converts implicitly from `string`, `Guid`, and `EventSourceId`, so you can pass the id you already have.

## Get all instances

`GetInstances` returns every instance in one call — read from the sink for a materialized read model, replayed from the event log for a passive one. Either way the whole set comes back at once, so filter the result with LINQ and keep it for small sets:

<ChronicleClientTabs snippet="scenarios/real-time-query/get-all" />

## Page through materialized state

For list views and grids, skip the replay: `Materialized` reads the instances the projection has already persisted to the sink — eventually consistent (typically milliseconds behind), fast regardless of history, and paged:

<ChronicleClientTabs snippet="scenarios/real-time-query/materialized-page" />

## Watch every change

`Watch<T>()` returns an `IObservable<ReadModelChangeset<T>>` that emits whenever **any** instance of that read model type changes — the changeset carries the key, the new state, and whether the instance was removed:

<ChronicleClientTabs snippet="scenarios/real-time-query/watch-changes" />

Dispose the subscription when you're done. If you need explicit lifecycle control — for example awaiting the watcher's `Subscribed` before producing events — use `GetWatcherFor<Book>()` instead.

## Observe a live page

`Materialized.ObserveInstances` pushes a fresh page snapshot whenever the stored data changes — a live dashboard or table with no polling:

<ChronicleClientTabs snippet="scenarios/real-time-query/observe-page" />

When a command appends `BookBorrowed`, the projection updates the materialized `Book` and every subscriber receives the new page. The brief gap between append and push is the normal [eventual consistency](../read-models/consistency.md) window — usually imperceptible.

:::note
Serving these to a frontend? Arc turns read models into observable queries with generated TypeScript proxies — drop one straight into a live table with [Displaying data](/components/displaying-data/).
:::

## Pick by consistency

| You need | Use | Consistency |
|---|---|---|
| One instance, exact current state | `GetInstanceById<T>(key)` | Strong |
| All instances, exact current state | `GetInstances<T>()` | Strong |
| A page for a list or grid | `Materialized.GetInstances<T>(skip, take)` | Eventual |
| To react to every change of a type | `Watch<T>()` | Pushed as changes apply |
| A live-updating page | `Materialized.ObserveInstances<T>(skip, take)` | Eventual, pushed |

## See also

- [Consistency models](../read-models/consistency.md) — the strong-vs-eventual trade-off in depth.
- [Getting a single instance](../read-models/getting-single-instance) and [Getting a collection of instances](../read-models/getting-collection-instances) — caching, performance, and the non-generic forms.
- [Materialized read models](../read-models/materialized-pagination) — pagination details and when to query the sink directly.
- [Watching read models](../read-models/watching-read-models) — changesets, filtering, and error handling.
