...
:::tip
You *can* read state from a reactor — just mind the consistency level. A **materialized** read model (what the projection writes to the sink, and the default) is [eventually consistent](../read-models/consistency.md) and may not have caught up with the very event you're reacting to — whether you read it through `IReadModels.GetInstanceById` or take it as a handler parameter. Mark the read model `[Passive]` when the reaction needs it to include the event in your hand: Chronicle then computes it from the event log on every read.
:::
