...
Events are **immutable facts**. Rewriting old events to a new shape would be falsifying the record — and it breaks the guarantee everything else depends on. Instead, you describe how to *read* old events as the new shape: an [event type migration](../concepts/event-type-migrations). The stored events stay exactly as written; Chronicle upgrades them on the way out.

## Do it

1. Define the new shape of the event type.
2. Register an **event type migration** that maps the previous generation to the new one — renaming, splitting, joining, or supplying a default for a new field (see [Event Type Migrations](../concepts/event-type-migrations) for the typed builder API).
...
- [Event Type Migrations](../concepts/event-type-migrations) — the full migration API and operations.
