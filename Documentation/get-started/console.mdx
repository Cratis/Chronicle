...
Add a reference to the [Chronicle client package](https://www.nuget.org/packages/Cratis.Chronicle), and the MongoDB driver for the step that queries the read models' database directly:
...
The `GetStartedBooks` query above reads documents Chronicle wrote. With no container to hand you an `IMongoCollection<GetStartedBook>`, you create one yourself: connect a `MongoClient` to `mongodb://localhost:27017`, open the `Quickstart` database, and get the `GetStartedBooks` collection. For the driver to deserialize those documents, it also needs to match how Chronicle stores them — register these conventions once at startup, before the first query:

[!INCLUDE [mongodb](./_includes/mongodb.mdx)]

With the conventions registered, the `GetStartedBooks` query reads the projection's documents exactly as written.

## Recap

You wired Chronicle into a bare console app by hand: created a `ChronicleClient`, opened the `Quickstart` event store, appended events for a small library domain, projected them into the `GetStartedBook` and `GetStartedBorrowedBook` read models with model-bound attributes, queried them through Chronicle and straight from the MongoDB sink, and reacted to one with a reactor. Because there was no DI container, every connection was explicit and in plain sight.
