...
We'll build the same small library domain as the other host guides. If you're building a web API instead, the [ASP.NET Core guide](./aspnetcore) covers that host; for the bare-bones, no-container version, see the [console guide](./console).
...
The projections and the `GetStartedBookReturnedNotifier` reactor pick those events up from the kernel — the worker stays alive to keep processing them.
...
Then register the database and the collection so a type can take an `IMongoCollection<GetStartedBook>` dependency:

<ChronicleClientTabs snippet="get-started/worker/mongo-registration" />

## Register your artifacts

Chronicle creates its discovered artifacts — reactors, reducers, projections — through the container. If an artifact is registered as a service, Chronicle uses that registration; otherwise it constructs the artifact itself and resolves its constructor parameters from the container. So you never have to register the reactor, but you do have to register what it depends on, such as the notification service a real waitlist reactor would take. As the solution grows, let Cratis Fundamentals register those services by convention:
...
- **Multi-tenant namespaces** — provide a custom `IEventStoreNamespaceResolver` to route operations per tenant. See [Namespace resolution](../namespaces/dotnet-client).
- **Structural dependencies** — supply custom identity providers, correlation-id accessors, or namespace resolvers through the `configure` callback on `AddCratisChronicle`. See [Structural dependencies](../configuration/structural-dependencies).
...
- **A different host** — the same artifacts run unchanged behind a web API ([ASP.NET Core](./aspnetcore)) or with no container at all ([console](./console)).
