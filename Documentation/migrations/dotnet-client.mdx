...
<ChronicleClientTabs snippet="migrations/dotnet-client/generations" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/migrator" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/split" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/combine" variants="csharp" />

### RenamedFrom

Maps a property from its old name in the source generation to its new name in the target generation.

<ChronicleClientTabs snippet="migrations/dotnet-client/renamed-from" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/default-value" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/nested-property" variants="csharp" />

A typed expression such as `e => e.Price.Description` identifies CLR members. Chronicle resolves each member to its JSON name: `[JsonPropertyName]` takes precedence, otherwise the client's JSON naming policy applies. Without explicit names, a camelCase policy produces `price.description`. Chronicle resolves that payload path segment by segment: reading walks into the object, writing creates the objects along the way when the stored payload does not carry them yet. That also lets a migration restructure a payload — `RenamedFrom(m => m.Price.Amount, e => e.Amount)` moves a value that used to sit at the top level into the object it now belongs to.

This resolution applies to typed source and target expressions for every operation, including bidirectional value maps. Raw string paths on `IEventMigrationPropertyBuilder` are already JSON paths and remain **exactly as supplied**. For example, a raw path `price.description` is not converted again, even with a custom naming policy. Use the exact names in the serialized payload, including explicit `[JsonPropertyName]` names. Existing raw-string migrations need no naming-policy upgrade workaround.

Separators, default values, and mapped values are literals and are not renamed. These builders describe property paths, not arbitrary custom-converter transformations.
...
<ChronicleClientTabs snippet="migrations/dotnet-client/no-operations" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/map-values" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/map-values-one-direction" variants="csharp" />
...
<ChronicleClientTabs snippet="migrations/dotnet-client/multi-generation" variants="csharp" />
