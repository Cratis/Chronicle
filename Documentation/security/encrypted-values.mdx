...
`EncryptionScope.Namespace` provisions one key shared by every `[Encrypted(EncryptionScope.Namespace)]` value in a given event store namespace, regardless of which document it came from — useful for a secret that belongs to the deployment rather than to any one entity, such as a webhook signing secret shared across every partner integration.

`EncryptionScope.Global` provisions one key shared by every `[Encrypted(EncryptionScope.Global)]` value across the whole installation — every event store, every namespace.
...
```csharp
public record PartnerApiKey(string Value) : ConceptAs<string>(Value);       // one key per partner (EncryptionScope.Subject)
