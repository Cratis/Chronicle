---
title: "Version Patch System"
description: "Write kernel patches that upgrade stored state between Chronicle versions, and how they are applied."
---
...
- **Declares a reverse step** through `Down()` — which Chronicle does not run for you; see below
...
Implement `Down()` so that the reverse of each patch is written down and reviewed alongside it. Chronicle only ever applies `Up()`: nothing calls `Down()` automatically, and running an older Chronicle version against patched storage does not undo a patch. Rolling back means restoring a backup taken before the upgrade:
