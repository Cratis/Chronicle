...
> Note: Notice the `<PackageReference Include="Cratis.Chronicle"/>`, its version is defined in the central package management (`Directory.Packages.props`) with a variable `$(Version)`
> During publishing, we use a build property setting this variable to the correct version number. This package reference is needed for the generated `.nuspec` to be correct when
