...
A shared page that uses `ChronicleClientTabs` must itself be `.mdx`. Put every placeholder on its own line in the self-closing form shown below. The documentation sync deliberately recognizes only that shape, so an inline, multiline, or paired component is not expanded.
...
The snippet id is extensionless. Add the same id under each participating client repo. A partial tab set is valid: a client participates only when that client should have a tab for this example.
...
Do not use an absent snippet to claim that a participating client lacks support. When the shared workflow should be visible for that client but its SDK does not support it yet, keep the tab visible with an explicit unsupported snippet:
