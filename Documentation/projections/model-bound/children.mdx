...
Disabling auto-mapping for a child type works in .NET and in TypeScript (`@noAutoMap` on the child type, from `@cratis/chronicle` 6.15.0). Kotlin, Java, and Elixir model-bound child collections do not expose a `NoAutoMap`-equivalent switch yet.
:::
...
## Collections with no children

Until a child event arrives the collection has no children, and Chronicle stores that as an absent field rather than
as `[]` — so that a parallel replay cannot erase a child a sibling event already added. How you declare the property
decides what you read back: non-nullable materializes as an empty collection, nullable stays `null` so you can tell
"no children yet" apart from "an empty list". See [Empty child collections](/chronicle/read-models/empty-child-collections/).
