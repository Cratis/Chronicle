...
In this example, an account can be removed by an `AccountClosed` event or an `AccountMerged` event when it is the source account.

## RemovedWithJoin

`RemovedWithJoin` on a root class is **not yet supported**. The engine logs a warning and does not subscribe to the removal event. Do not add this attribute to a root read model: it will not delete its instances. Use `RemovedWith` for root deletion, or use `RemovedWithJoin` on a child collection for child-item removal.
...
This example shows supported root `RemovedWith` and child `RemovedWith` declarations:
