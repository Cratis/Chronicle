...
Clearing means returning a member to *no value*, so the member must have that state. A non-nullable member does not, and the declaration is refused: [CHR0048](../../code-analysis/CHR0048) reports it as a build error, and building the projection throws `CannotClearNonNullableMember`.
...
`ToValue(null)` builds the same clear:

```csharp
public class MbClearingProjectNotesProjection : IProjectionFor<MbClearingProjectNotes>
{
    public void Define(IProjectionBuilderFor<MbClearingProjectNotes> builder) => builder
        .From<MbClearingProjectNoted>(_ => _
            .Set(m => m.Note).To(e => e.Note))
        .From<MbClearingProjectNoteCleared>(_ => _
