...
- Model the property as a collection of one item using [children](declarative/children), which forces a key, breaks the scalar nature of the property, and complicates queries; or
...
`nested` solves both problems with a small, dedicated primitive that mirrors the shape of [children](declarative/children) without the collection semantics — one scalar, nullable property, with explicit set and clear lifecycle events.
...
- **Recursive** — a nested object can contain event-specific and every-event mappings, `remove with`, and further nested objects. A nested object outside a child collection can also contain `join`. The declaration language rejects `children`, `all`, and `remove via join` inside `nested`, and rejects `join` in a nested object under `children`. Declare collections at the parent level instead.
...
The contract is intentionally narrow: nested is *not* a soft-deletion marker for the parent, and it is *not* a workaround for one-of-many relationships — use [children](declarative/children) for those.
...
A `nested` block is declared with a property name. It supports `from`, `every`, `remove with`, and `clear with`, and can contain further `nested` blocks. `join` is supported only when the nested object is not under `children` (including any deeper nested level there). It does not support `all`, `children`, or `remove via join` inside the nested block:
...
Two new keywords are introduced: `nested` (block opener) and `clear with` (lifecycle directive). Both can appear inside `children` and inside other `nested` blocks. See [PDL Nested Objects](/screenplay/projections/nested) for the full surface.
...
The nested type is scanned for `[FromEvent<T>]`, `[ClearWith<T>]`, `[SetFrom<T>]`, `[AddFrom<T>]`, and `[Nested]`. Do not declare `[ChildrenFrom<T>]` or `[RemovedWithJoin<T>]` inside a nested scalar object: those definitions are not wired by the engine, and the engine logs a warning. A model-bound or fluent `Join` inside a nested object under children also logs a warning and is not wired. See [Model-Bound Nested Objects](model-bound/nested).

### Recursive nesting

A `nested` block can appear inside another `nested` block, inside a `children` block, and inside a child of a child — there is no recursion depth limit imposed by the model. The reverse shape, `children` inside `nested`, is not supported. Each layer is rendered into the projection definition as a `ChildrenDefinition` with `IdentifiedBy = PropertyPath.NotSet`, which the engine treats as "scalar" rather than "collection".
...
A `nested` block may also live inside a [children](declarative/children) collection, attaching a single nullable child object to every item in the collection. The engine resolves the property path with the child's array indexers so each item maintains its own nested state. Joins inside that nested object (including deeper nested objects) are not supported: live join events cannot yet locate the correct child by its nested join property. PDL rejects them; the model-bound and fluent paths log a warning and omit their subscriptions:
...
| `Children` | Nested collections within items | Not supported within the scalar |
...
- [PDL — Nested Objects](/screenplay/projections/nested)
- [Declarative — Nested Objects](declarative/nested)
- [Model-Bound — Nested Objects](model-bound/nested)
- [Children](declarative/children)
