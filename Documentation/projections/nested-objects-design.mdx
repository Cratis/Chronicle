...
Two new keywords are introduced: `nested` (block opener) and `clear with` (lifecycle directive). Both can appear inside `children` and inside other `nested` blocks. See [PDL Nested Objects](projection-declaration-language/nested) for the full surface.
...
- [PDL — Nested Objects](projection-declaration-language/nested)
