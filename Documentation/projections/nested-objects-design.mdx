...
- **Recursive** — a nested object can contain joins, event-specific and every-event mappings, and further nested objects. The declaration language rejects `children`, `all`, and `remove via join` inside a `nested` block. Declare a collection at the parent level instead.
...
A `nested` block is declared with a property name. It supports `from`, `every`, `join` and `clear with`, and can contain further `nested` blocks. It does not support `all`, `children`, or `remove via join` inside the nested block:
...
The nested type is scanned for `[FromEvent<T>]`, `[ClearWith<T>]`, `[SetFrom<T>]`, `[AddFrom<T>]`, and `[Nested]`. Do not declare `[ChildrenFrom<T>]` or `[RemovedWithJoin<T>]` inside a nested scalar object: those definitions are not wired by the engine. See [Model-Bound Nested Objects](model-bound/nested).
...
A `nested` block may also live inside a [children](declarative/children) collection, attaching a single nullable child object to every item in the collection. The engine resolves the property path with the child's array indexers so each item maintains its own nested state:
