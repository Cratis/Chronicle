...
description: "How the projection engine compiles model-bound, declarative, and PDL projections and materializes read models."
---
...
graph TB
    subgraph Input["Projection Declaration Approaches"]
        ...
        ModelBound["Model-Bound (Attributes/Annotations)"]
        Declarative["Declarative (Fluent API)"]
    end
...
```pdl
projection MyProjection => MyModel
  from MyEvent
    name = eventProperty
    count eventCount
```
...
Declarative projections use a fluent API to define projections programmatically with maximum flexibility.
...
graph LR
    ...
    subgraph Compiler["Compilation Layer"]
        Screenplay["Screenplay Compiler (Cratis.Screenplay)"]
        Validator["Semantic Validator"]
        Visitor["ProjectionDefinition Visitor"]
    end
    ...
    MonacoEditor --> |Declaration Text| Screenplay
