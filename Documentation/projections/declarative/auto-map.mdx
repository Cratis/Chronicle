...
This scoped re-enable is C#-only. Kotlin and Java have child collections in their projection builders, but the child builder has no AutoMap switch, so there is no child scope to re-enable AutoMap on. Elixir and TypeScript do not yet support declarative child collections (see [Children](./children)).
:::
