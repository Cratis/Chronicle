...
> **Note:** AutoMap is enabled by default at the top level. You don't need to call `.AutoMap()` explicitly unless you want to ensure it's enabled in contexts where it might be disabled. Read more about [auto mapping](./auto-map)
...
Auto-mapping works when property names and types match between the event and read model. For more control over the mapping, see [projection with custom properties](./set-properties).
