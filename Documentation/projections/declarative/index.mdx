...
description: "Define projections in code with the fluent IProjectionFor<T> builder."
---
...
- **Variants** for mutually exclusive read model shapes of the same logical entity
...
| Recipe | Description |
| ------ | ----------- |
| [Simple projection](simple-projection) | Basic projection using AutoMap() |
| [AutoMap](auto-map) | Automatic property mapping at different levels |
| [Passive](passive) | In-memory projections for on-demand lookups |
| [Set properties](set-properties) | Explicit property mapping and transformations |
| [Children](children) | Hierarchical data models with child collections |
| [Joins](joins) | Cross-stream projections using joins |
| [Functions](functions) | Arithmetic and other functions |
| [Composite keys](composite-keys) | Multi-property key identification |
| [Event context](event-context) | Using event metadata in projections |
| [FromEvery](from-every) | Setting properties for all events in a projection |
| [FromAll](from-all) | Subscribe to all event types without filtering |
| [Initial values](initial-values) | Default values for read model properties |
| [RemoveWithJoin](remove-with-join) | Cross-stream child removal |
| [FromEventSequence](from-event-sequence) | Sourcing events from specific event sequences |
| [NotRewindable](not-rewindable) | Forward-only projections that cannot be replayed |
