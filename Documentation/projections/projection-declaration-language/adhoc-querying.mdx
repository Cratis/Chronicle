...
Ad-hoc PDL querying with `IProjections.Query()` is currently C#-only. Kotlin's `IProjectionsService`, Elixir's `Chronicle.Projections.Projection`, and TypeScript's `IProjections` all support only statically-registered/annotated projections — none exposes a runtime API for sending a PDL declaration string and getting back projected results.
:::
