...
By default Chronicle **materializes** the projection into its configured **sink** storage — MongoDB unless you change it — so the `Book` read model is just a collection you query, exactly what you're used to. It lives in a collection named after the read model, pluralized (`Books`), in the database named after your event store; [the MongoDB setup in Get started](/chronicle/get-started/console/#configure-the-mongodb-client) shows how to point the driver at it:
...
You can now see the catalog. The last piece is to make the library *do* something when the world changes — when a book comes back, tell the next person waiting for it. That's a job for a reactor, and it's the [final chapter](./reacting). [Let's finish the tour →](./reacting)
