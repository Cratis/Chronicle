...
A fact you can't query, though, isn't much use yet — right now the book exists only as history. In the [next chapter](./read-model) we'll fix that: we'll teach Chronicle to fold this stream of events into a `Books` read model you can actually query, and watch it update itself as more events arrive. [Onward →](./read-model)
