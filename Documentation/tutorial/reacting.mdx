...
- **Solve a focused task** — use the [Scenarios](/chronicle/scenarios/) recipes. [Test a slice](/chronicle/scenarios/test-a-slice/) shows how `EventScenario` and `ReadModelScenario` verify event appends and derived read model state in-process.
