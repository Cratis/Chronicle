---
title: "Connection string elements"
description: "The Chronicle connection string grammar a client must implement: scheme, hosts, credentials, and every query parameter."
---
...
worth offering alongside the parser. The shape worth matching — the .NET client's own builder,
used to construct a connection string from discrete settings:
...
`WithCertificate(path, password)`. A new client's `Build()` should validate as it assembles — the same
"credentials and API key together is an error" rule the parser enforces should be enforced there
too, at construction time rather than at first-connect time. The .NET builder does not do this yet:
it reports the conflict only when the authentication mode is first read.
