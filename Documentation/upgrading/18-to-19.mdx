...
<ChronicleClientTabs snippet="upgrading/18-to-19/skip-compatibility-check" />
...
:::caution[19.4.8 and later also run Chronicle's analyzers in your build]
From 19.4.8, the `Cratis.Chronicle` package brings the `CHR` code analysis rules with it;
earlier packages did not, so for most applications this rebuild is the first time the rules
run. Code that violates an Error rule stops compiling, and under `TreatWarningsAsErrors` so
does code that violates a Warning rule. This applies to any upgrade that lands on 19.4.8 or
later, including one from an earlier 19.x. Fix what the build reports; see
