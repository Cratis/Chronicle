...
- **Values can be released.** When the thing that held the value goes away, mark that event with `[RemoveConstraint]` so the value can be claimed again. A lifecycle that ends in more than one way marks each of those events — every one of them releases the value:
...
**Note:** The hash transformation is transparent to your code — you declare constraints on plain properties, and storage uses hashes under the hood. The same instinct shapes what a violation says: Chronicle's own message names the constraint and the offending *property*, never the value it collided on. A protected value is therefore never echoed back by a message Chronicle composed — and the caller doesn't need it echoed, because it is the value it just tried to append.
...
- A **constraint** protects a *value* across every event source in an event sequence — no two users in the event log may ever hold the same email, no matter who appends or when. The check is scoped to one namespace and one event sequence: two tenants in different namespaces can hold the same value, and so can two different event sequences.
