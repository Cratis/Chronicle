---
title: Chronicle
description: The event sourcing platform and framework with a gRPC boundary, a first-class .NET client, TypeScript, Kotlin/Java, and Elixir clients — Python coming soon — storage backends for MongoDB/PostgreSQL/SQL Server/SQLite, and rich tooling.
---
...
<CardGrid>
  ...
  <SimpleCard title="Choose an application host model" icon="puzzle" link="/chronicle/get-started/choose-hosting-model/">
    Compare console, worker, and ASP.NET Core hosts — with Arc as a layer on top — and run the kernel locally or under Aspire.
  </SimpleCard>
...
<CardGrid>
  <SimpleCard title="gRPC at the boundary" icon="puzzle" link="/chronicle/architecture/">
    Chronicle is a client-server kernel with gRPC/protobuf contracts. The .NET client is first-class; TypeScript, Kotlin/Java, and Elixir clients are built on the same protocol, with a Python client coming soon.
  </SimpleCard>
  ...
  <SimpleCard title="Processing lives in the kernel" icon="rocket" link="/chronicle/architecture/">
    Distributed, stateful processing is the kernel's job: event sequences, observers, jobs, and recovery run inside the runtime, not in your application code.
  </SimpleCard>
...
<CardGrid>
  ...
  <SimpleCard title="Data integrity" icon="approve-check" link="/chronicle/understanding-constraints/">
    Enforce uniqueness with kernel-side constraints and guard racing appends with concurrency scopes — both checked before commit.
  </SimpleCard>
