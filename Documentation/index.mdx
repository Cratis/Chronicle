...
Chronicle stands on its own — run it from any .NET host, with no other Cratis product required. When you want a typed full-stack app on top, [Arc](/arc/) builds on Chronicle and [Components](/components/) renders it. See [Why Cratis](/why-cratis/) for how the pieces fit — and which to reach for first.
