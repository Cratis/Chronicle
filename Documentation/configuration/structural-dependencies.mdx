...
description: "Supply custom resolvers and providers to the .NET client, and how they differ from options bound from configuration."
---
...
- `configureOptions` feeds the options pipeline. It runs after `appsettings.json` and `IConfiguration` are bound, so a value it sets overrides the configured one.
