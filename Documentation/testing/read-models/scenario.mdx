...
`Compliance` is worth singling out, because it is the one whose absence looks most like success. A deployed Chronicle encrypts a read model's `[PII]` members on the way into the sink and releases them on the way out, keyed by the subject the event resolved to. `ReadModelScenario` projects and reads plaintext, so a value that a running system would store as ciphertext — or hand back empty because the subject was erased — reads back here exactly as you wrote it. Assertions about the *value* still hold; assertions about it being protected, about which subject it is keyed by, or about what erasure does to it, do not. Cover those with an `EventScenario`, which does run the real compliance stack, or against a real kernel and store.
