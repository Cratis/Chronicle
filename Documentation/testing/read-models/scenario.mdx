...
events directly. The optional initial read model state follows a separate conversion path; do not pass an
initial state containing nonempty `JsonObject` values, because that path still reflects over their parent
links.
