...
Without a configuration callback, the subscription forwards every event type the client knows: the event types registered by the application that creates the subscription. Use `WithEventType<T>()` to limit the subscription to specific types:

<ChronicleClientTabs snippet="subscriptions/explicit-subscriptions/filtering" />

To forward every event type the client knows without filtering, omit the configuration callback entirely:
