...
A reactor can be handed the same event more than once, and until now it had no way to tell the difference. Declare
a `ReactorDelivery` parameter on a handler method and Chronicle gives you a stable identity for *this delivery of
this event to this reactor* — the same value every time the event comes back, and a different value for every
genuinely different delivery.
...
| | Covers replay | Covers recovery after a failure | Needs storage |
|---|---|---|---|
...
- **The gap between the effect and the record is still a gap.** Charge the card, and die before writing the
  receipt, and the retry charges again. Recording the identity narrows at-least-once towards at-most-once exactly
