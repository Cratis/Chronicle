...
Chronicle registers its built-in handlers and reactor metadata providers when you call `AddCratisChronicle`, including in standalone hosts. Handlers discovered from other loaded libraries only participate if that library has registered them with the host; merely loading an assembly does not activate its integration.
