---
title: "Event Redaction"
description: "Remove an event's content from the log with redaction, what it records, and how observers see it."
---

Redaction removes the meaningful content of a previously appended event when you need to erase data — for example, to comply with a right-to-erasure request. Rather than deleting the event entirely, Chronicle replaces the event's payload with a redaction marker that preserves the sequence number and limited audit metadata. The original event type, occurrence, correlation ID, caused-by identities, and causation types and times remain; original causation property values, revision history, and content hashes do not.
...
- The **original** `Occurred`, `CorrelationId`, and `CausedBy` identity chain.
- The original `Causation` entries' types and occurrence times, **without** their property values.

The event's own context fields reflect the redaction action. The original payload, revision history, and content hashes are removed from event sequence storage. Redaction cannot reach copies made before redaction, such as database oplogs or backups, or events already forwarded to other event stores through outbox/inbox. Handle those copies separately.
...
| `Causation` | The original causation types and occurrence times, without property values. |
...
Confirming records the redaction request; a built-in reactor then replaces the event's content with the redaction marker, usually within moments. The event slot remains visible in the sequence but no longer contains the original data. Before you report the data as removed, reopen the event and check that its content has been replaced.
