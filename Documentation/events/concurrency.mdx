...
flowchart TD
    ...
    T -->|no| U["Unavailable — nothing to compare against"]
    U --> S[Check skipped, warning logged]
    S --> K
```
...
When two writers must not both create the first event in a scope, enforce that with a [constraint](../constraints/index.md) rather than a concurrency scope. A unique event type constraint permits exactly one event of a given type per event source, and the kernel rejects the second append no matter which client made it.

## Best Practices

1. **Use specific scoping**: Scope concurrency as narrowly as possible to avoid unnecessary blocking — but note that the narrower the scope, the more scopes there are, and [the first append into each one is not checked](#the-first-append-into-a-scope-is-not-checked)
