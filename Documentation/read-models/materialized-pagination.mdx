...
- **SQL (PostgreSQL, SQL Server, SQLite)** — reads the current page on subscription, then polls the stored rows for changes. Pages are ordered by the read model key. Several writes between polls can appear as one updated snapshot; use `IReadModels.Watch<T>()` if you need each projection changeset. If observation is unavailable, load the page with `Materialized.GetInstances<T>()` and apply the changesets from `Watch<T>()` to keep it current.
