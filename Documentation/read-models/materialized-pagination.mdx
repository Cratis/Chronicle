...
- **SQL (PostgreSQL, SQL Server, SQLite)** — reads the current page on subscription, then polls the stored rows and total count for changes. Pages are ordered by the read model key. Set `Cratis:Chronicle:Sql:LiveQueryPollIntervalSeconds` to change the polling interval (default: 2 seconds). Several writes between polls can appear as one updated snapshot; for projection-backed read models, use `IReadModels.Watch<T>()` if you need each projection changeset.

> [!NOTE]
> Always dispose observation subscriptions when the consumer is torn down to close change streams or stop SQL polling.
...
|---|---|---|
| **Data source** | The sink for a materialized read model; an event log replay for a passive one, or when you pass an event count | Materialized sink (database) |
| **Consistency** | Eventual for a materialized read model; strong for a passive one | Eventual — milliseconds behind |
| **Performance** | Reads every stored instance, or replays every event for a passive one | O(1) — direct database lookup |
| **Pagination** | No | Yes — skip/take |
| **Large datasets** | Slow — returns everything in one call | Fast — loads only the requested page |
