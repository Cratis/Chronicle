...
If none of the three resolve to a value, `Release` returns the instance unchanged and logs a warning when the model has `[PII]` or `[Encrypted]` metadata. If the read model has neither, it returns the instance unchanged without a warning because there is nothing to release.
