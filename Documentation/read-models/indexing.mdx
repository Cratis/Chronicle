...
which properties are indexed, and Chronicle creates them every time the container is built.

Today that applies to the MongoDB sink. The SQL sink creates its tables but not the indexes a read
model declares, so on SQL storage you still create them yourself.
