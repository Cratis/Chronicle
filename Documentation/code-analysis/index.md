...
| [CHR0035](CHR0035) | Read model declares a reserved compliance subject property | Error | Chronicle reserves `_subject`, `__subject`, and `__subjects` for internal compliance-subject tracking; a same-named property collides with stored metadata |
...
| [CHR0048](CHR0048) | A clear is declared for a member that cannot hold null | Error | Clearing means returning a member to no value; a non-nullable member has no such state, so the only thing the projection could write is the type default - a different fact the read model cannot tell apart from a real value |
