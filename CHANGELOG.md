...
[Microsoft documentation](https://learn.microsoft.com/en-us/dotnet/standard/garbage-collection/workstation-server-gc) recommends using a server GC in a multi-processor(core) environment. Our findings is that for a serialization heavy solution like the Cratis Kernel, this does not seem to apply. Garbage collection happens at a too low priority and we end up eating more memory than needed and the garbage collector not getting around to collect before we've reached hundreds of megabytes and in some cases gigabytes. Collection at that time is then very slow, as it has to collect a lot. But the unmanaged heap does not get collected straight away and could take up to hours before it is fully collected and stabilized.
