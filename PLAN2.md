...
## A third collision, found during spec verification: `EventContext.Hash`/`ObservationState` in the Sequences read path

**Found 2026-08-25, while chasing down two failing `Integration/Client` specs** (`for_EventSequence.when_revising.an_event*.should_have_a_hash_set`) as part of a full-repo spec sweep. Confirmed via an `origin/main` worktree comparison that these two tests pass there and fail only on this branch — a real, reproducible gap, not flakiness.

**Root cause:** `Source/Kernel/Core/Sequences/EventContext.cs` (the Core artifact backing the generated `Contracts.Sequences.EventContext`, from this branch's `Sequences` service work) carries no `Hash`/`ObservationState` fields at all. `Source/Clients/DotNET/Events/EventContextConverters.cs`'s `ToClient(this Contracts.Sequences.EventContext ...)` overload already has an inline `<remarks>` acknowledging this and hard-codes `EventHash.NotSet`/`EventObservationState.Initial`. The kernel/storage layer's own hash computation and persistence for `Revise` is untouched and correct (byte-identical to `origin/main` across `Core/EventSequences/**` and `Storage.MongoDB/EventSequences/**`) — this is purely a read-path gap: **every** event read through the new `Sequences` service reports the wrong `Hash`/`ObservationState`, not just revised ones; the failing spec is just the one place anything happens to assert on `Context.Hash`.

**Why this isn't a quick fix, and follows the same shape as the `ConcurrencyScope` collision above:** the obvious fix — add `Hash`/`ObservationState` properties to `Core/Sequences/EventContext.cs`, typed as the existing `Concepts.Events.EventHash`/`Concepts.Events.EventObservationState` (Core already half-owns both under `Concepts.*`, exactly the case Phase 4 step 1 says to reuse rather than re-invent) — walks straight into the same trap. `Concepts.Events.EventObservationState` maps, via the same skip-2/base-namespace transform that turned `Concepts.Events.EventType` into a collision with `Contracts/Events/EventType.cs` above, onto `Contracts/Events/EventObservationState.cs` — which **already exists, hand-written, and is not the same shape**: the hand-written version has 5 members (`None, Initial, HeadOfReplay, Replay, TailOfReplay`) where `Concepts.Events.EventObservationState` has 3 (`None, Initial, Replay`), and `Replay`'s bit value differs between them (`1<<2` vs `1<<1`). That hand-written enum is a live dependency of `Core/Captures/EventContextConverters.cs` and both the Grpc- and Client-side `EventObservationStateConverters.cs` — regenerating it from the 3-member Concepts enum would either proto-collide immediately (best case) or silently truncate two flag values out from under Captures and the still-hand-written observation converters (worst case, per the exact "if they don't collide, there is no signal at all" warning above). This was not tested live — the risk was identified by inspection and the fix was **not attempted**, per the standing instruction to stop rather than push a `SharedTypeRegistry`-adjacent change through unverified.

**Status: an accepted, documented gap for this pass, not fixed.** Whoever wired the `Sequences` read path most likely hit or anticipated exactly this and hard-coded the defaults deliberately rather than risk the collision — the inline `<remarks>` on `EventContextConverters.ToClient` is consistent with that, even though (unlike `ConcurrencyScope`) it was never elevated to an explicit owner directive in this file. Recording it here now so it has the same visibility.

**Before fixing this for real:** the two `EventObservationState` enums need reconciling first — either the hand-written one's extra two flags (`HeadOfReplay`, `TailOfReplay`) get added to `Concepts.Events.EventObservationState` (checking every existing consumer of the Concepts version tolerates two new flag values) so one type can honestly replace the other, or the Core `Sequences.EventContext` needs its own distinctly-named property/type that deliberately avoids `SharedTypeRegistry` picking up the collision-prone `Concepts.Events.EventObservationState` reference at all. Either path needs the same rigor as the `ConcurrencyScope` follow-up: enumerate every consumer of `Contracts/Events/EventObservationState.cs` first (`Core/Captures/EventContextConverters.cs`, `Source/Kernel/Grpc/Events/EventObservationStateConverters.cs`, `Source/Clients/DotNET/Events/EventObservationStateConverters.cs`), a proto diff, and `WireCompatibility` — not a routine change.

**Resolved, 2026-08-26 — directive from the project owner: the hand-written enum is not the real one.** `Concepts.Events.EventObservationState` is authoritative; `Contracts/Events/EventObservationState.cs` is meant to be its generated mirror, collision and all. Before wiring it, every one of the checks this section called for was actually run, not assumed:

- Every real consumer of `Contracts.Events.EventObservationState` was grepped repo-wide: only the three files this section already named, and all three already only switch on `None`/`Initial`/`Replay` — none of them, anywhere, ever reference `HeadOfReplay` or `TailOfReplay`. A second grep for those two identifiers by name, across the whole repo, matched nothing outside the hand-written enum's own declaration. The two extra flags were dead weight with no live reader, which is what made the collision genuinely safe rather than merely convenient.
- `Core/Sequences/EventContext.cs` now takes `Hash`/`ObservationState` (typed as the `Concepts.Events` originals), threaded through from `Concepts.Events.EventContext` (which already carried real, populated values throughout the storage layer — the gap was purely that the `Sequences` read path dropped them, exactly as suspected) to the wire and back down to the client SDK.
- Rebuilding Core hit the expected bootstrap ordering problem (Core's own `ToContract()` needed the new Contracts fields to exist before they could exist) — resolved the same way `PLAN.md`'s "Renames and the build cycle" describes: strip the new assignment, build once to let the generator observe the new Core fields and regenerate `Contracts/Events/EventObservationState.cs` (now `<auto-generated />`, 3 members, `None=0/Initial=1/Replay=2`) and `Contracts/Sequences/EventContext.cs` (now carrying `Hash`/`ObservationState`), then restore the assignment and build again clean.
- The proto diff came out exactly as predicted: `captures.proto`, `observation_reactors.proto`, `observation_reducers.proto`, and `readmodels.proto` all lost the same two flag members and renumbered `Replay` from `4` to `2`, consistently, everywhere `Contracts.Events.EventObservationState` is inlined — one shared type, one consistent change, not a stray side effect.
- `WireCompatibility --major 16 --since 16.39.0` reported nothing to compare against — no release exists yet at or after the declared floor, so this entire branch (this change included) ships as the first thing under 16.39.0. There is no already-shipped wire contract this could break.
- Three `DotNET.Specs` fixtures that constructed a bare `Contracts.Sequences.EventContext` without setting `Hash` surfaced a real robustness gap in the client converter: it dereferenced `context.Hash` unconditionally, and `ConceptAs<T>`'s constructor throws on a null value. Fixed defensively (`context.Hash is null ? EventHash.NotSet : context.Hash`) rather than only patching the fixtures — a `string` wire field with no `IsRequired` can legitimately arrive unset from an older server during a rolling upgrade, so treating absence as `NotSet` is the correct behavior, not just a test accommodation.
- Full verification: `GrpcCodeGenerator.Specs` (69/69), `Core.Specs` (2831/2832, the one pre-existing skip), `DotNET.Specs` (2530/2530), and — the spec that started this — `Integration/Client/for_EventSequence/when_revising/*` against a real Docker-built kernel: 17/17, including both previously-failing `should_have_a_hash_set` facts.
