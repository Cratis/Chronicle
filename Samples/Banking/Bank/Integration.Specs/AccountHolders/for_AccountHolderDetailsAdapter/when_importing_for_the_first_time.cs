...
public class when_importing_for_the_first_time : given.object_ready_for_import
{
    ...
    async Task Because()
    {
        ...
        result = await context.Projection.GetById(event_source_id);
    }

    [Fact] void should_append_account_holder_registered() => context.ShouldAppendEvents(new AccountHolderRegistered(first_name, last_name, birth_date, new(address, postal_code, city, country), social_security_number));
