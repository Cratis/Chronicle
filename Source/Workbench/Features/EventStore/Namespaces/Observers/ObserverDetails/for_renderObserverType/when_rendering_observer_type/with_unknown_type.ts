...
describe('when rendering observer type with unknown type', () => {
    let result: string;

    beforeEach(() => {
        const observer = new ObserverInformation();
        observer.type = ObserverType.unknown;
        result = renderObserverType(observer);
    });

    it('should return the unknown label', () => {
        result.should.equal(strings.eventStore.namespaces.observers.types.unknown);
    });
});
