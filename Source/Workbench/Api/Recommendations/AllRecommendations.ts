// @generated by Cratis. Source: Cratis.Chronicle.Api.Recommendations.RecommendationQueries. Time: 2026-04-14T16:04:04.2330510Z. Hash: E844940507923C34F503596ECA006575B6FC98EB362E7EDD5F0C2F960AE815E0
/*---------------------------------------------------------------------------------------------
 *  **DO NOT EDIT** - This file is an automatically generated file.
 *--------------------------------------------------------------------------------------------*/

/* eslint-disable sort-imports */
// eslint-disable-next-line header/header
import { ObservableQueryFor, QueryResultWithState, Sorting, SortingActions, SortingActionsForObservableQuery, Paging, ChangeSet } from '@cratis/arc/queries';
import { useObservableQuery, useObservableQueryWithPaging, useSuspenseObservableQuery, useSuspenseObservableQueryWithPaging, useChangeStream, SetSorting, SetPage, SetPageSize, ObservableQueryWhen } from '@cratis/arc.react/queries';
import { ParameterDescriptor } from '@cratis/arc/reflection';
import { Recommendation } from './Recommendation';

class AllRecommendationsSortBy {
    private _description: SortingActionsForObservableQuery<Recommendation[]>;
    private _id: SortingActionsForObservableQuery<Recommendation[]>;
    private _name: SortingActionsForObservableQuery<Recommendation[]>;
    private _occurred: SortingActionsForObservableQuery<Recommendation[]>;
    private _type: SortingActionsForObservableQuery<Recommendation[]>;

    constructor(readonly query: AllRecommendations) {
        this._description = new SortingActionsForObservableQuery<Recommendation[]>('description', query);
        this._id = new SortingActionsForObservableQuery<Recommendation[]>('id', query);
        this._name = new SortingActionsForObservableQuery<Recommendation[]>('name', query);
        this._occurred = new SortingActionsForObservableQuery<Recommendation[]>('occurred', query);
        this._type = new SortingActionsForObservableQuery<Recommendation[]>('type', query);
    }

    get description(): SortingActionsForObservableQuery<Recommendation[]> {
        return this._description;
    }
    get id(): SortingActionsForObservableQuery<Recommendation[]> {
        return this._id;
    }
    get name(): SortingActionsForObservableQuery<Recommendation[]> {
        return this._name;
    }
    get occurred(): SortingActionsForObservableQuery<Recommendation[]> {
        return this._occurred;
    }
    get type(): SortingActionsForObservableQuery<Recommendation[]> {
        return this._type;
    }
}

class AllRecommendationsSortByWithoutQuery {
    private _description: SortingActions  = new SortingActions('description');
    private _id: SortingActions  = new SortingActions('id');
    private _name: SortingActions  = new SortingActions('name');
    private _occurred: SortingActions  = new SortingActions('occurred');
    private _type: SortingActions  = new SortingActions('type');

    get description(): SortingActions {
        return this._description;
    }
    get id(): SortingActions {
        return this._id;
    }
    get name(): SortingActions {
        return this._name;
    }
    get occurred(): SortingActions {
        return this._occurred;
    }
    get type(): SortingActions {
        return this._type;
    }
}

/**
 * Get and observe all observers for an event store and namespace.
 */
export interface AllRecommendationsParameters {
    
    /**
     * EventStore the recommendations are for.
     */
    eventStore: string;
    
    /**
     * Namespace the recommendations are for.
     */
    namespace: string;
}
/**
 * Get and observe all observers for an event store and namespace.
 */
export class AllRecommendations extends ObservableQueryFor<Recommendation[], AllRecommendationsParameters> {
    readonly route: string = '/api/event-store/{eventStore}/{namespace}/recommendations/all-recommendations/observe';
    readonly queryName: string = 'Cratis.Chronicle.Api.Recommendations.RecommendationQueries.AllRecommendations';
    readonly treatWarningsAsErrors: boolean = false;
    readonly roles: string[] = [];
    readonly defaultValue: Recommendation[] = [];
    private readonly _sortBy: AllRecommendationsSortBy;
    private static readonly _sortBy: AllRecommendationsSortByWithoutQuery = new AllRecommendationsSortByWithoutQuery();

    constructor() {
        super(Recommendation, true);
        this._sortBy = new AllRecommendationsSortBy(this);
    }

    get requiredRequestParameters(): string[] {
        return [
            'eventStore',
            'namespace',
        ];
    }

    readonly parameterDescriptors: ParameterDescriptor[] = [
        new ParameterDescriptor('eventStore', String, false),
        new ParameterDescriptor('namespace', String, false),
    ];

    eventStore!: string;
    namespace!: string;

    get sortBy(): AllRecommendationsSortBy {
        return this._sortBy;
    }

    static get sortBy(): AllRecommendationsSortByWithoutQuery {
        return this._sortBy;
    }

    static use(args?: AllRecommendationsParameters, sorting?: Sorting): [QueryResultWithState<Recommendation[]>, SetSorting] {
        return useObservableQuery<Recommendation[], AllRecommendations, AllRecommendationsParameters>(AllRecommendations, args, sorting);
    }

    static useWithPaging(pageSize: number, args?: AllRecommendationsParameters, sorting?: Sorting): [QueryResultWithState<Recommendation[]>, SetSorting, SetPage, SetPageSize] {
        return useObservableQueryWithPaging<Recommendation[], AllRecommendations>(AllRecommendations, new Paging(0, pageSize), args, sorting);
    }

    static useSuspense(args?: AllRecommendationsParameters, sorting?: Sorting): [QueryResultWithState<Recommendation[]>, SetSorting] {
        return useSuspenseObservableQuery<Recommendation[], AllRecommendations, AllRecommendationsParameters>(AllRecommendations, args, sorting);
    }

    static useSuspenseWithPaging(pageSize: number, args?: AllRecommendationsParameters, sorting?: Sorting): [QueryResultWithState<Recommendation[]>, SetSorting, SetPage, SetPageSize] {
        return useSuspenseObservableQueryWithPaging<Recommendation[], AllRecommendations>(AllRecommendations, new Paging(0, pageSize), args, sorting);
    }

    static useChangeStream(args?: AllRecommendationsParameters, getKey?: (item: Recommendation) => unknown, sorting?: Sorting): ChangeSet<Recommendation> {
        return useChangeStream<Recommendation, AllRecommendations, AllRecommendationsParameters>(AllRecommendations, args, getKey, sorting);
    }

    static when(condition: boolean): ObservableQueryWhen<AllRecommendations, Recommendation[], AllRecommendationsParameters> {
        return new ObservableQueryWhen<AllRecommendations, Recommendation[], AllRecommendationsParameters>(AllRecommendations, condition);
    }
}
