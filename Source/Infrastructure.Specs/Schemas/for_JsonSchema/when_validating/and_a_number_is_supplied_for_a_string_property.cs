...
public class and_a_number_is_supplied_for_a_string_property : Specification
{
    record EventWithStringProperty(string Name);

    IList<JsonSchemaValidationError> _result;

    void Because() => _result = JsonSchema.FromType<EventWithStringProperty>().Validate("""{"name":42}""");
    ...
    [Fact] void should_report_the_offending_property_path() => _result.Single().Path.ShouldEqual("name");
}
