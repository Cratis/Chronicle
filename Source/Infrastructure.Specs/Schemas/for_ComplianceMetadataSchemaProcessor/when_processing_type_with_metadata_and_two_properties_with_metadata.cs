...
public class when_processing_type_with_metadata_and_two_properties_with_metadata : given.a_processor_and_a_context_for<TypeWithProperties>
{
    ...
    void Establish()
    {
        ...
        _resolver.Setup(_ => _.HasMetadataFor(typeof(TypeWithProperties))).Returns(true);
        _resolver.Setup(_ => _.GetMetadataFor(typeof(TypeWithProperties))).Returns([first_type_metadata, second_type_metadata]);
        ...
        _resolver.Setup(_ => _.HasMetadataFor(TypeWithProperties.FirstProperty)).Returns(true);
        _resolver.Setup(_ => _.GetMetadataFor(TypeWithProperties.FirstProperty)).Returns([first_property_first_metadata, first_property_second_metadata]);
        ...
        _resolver.Setup(_ => _.HasMetadataFor(TypeWithProperties.SecondProperty)).Returns(true);
        _resolver.Setup(_ => _.GetMetadataFor(TypeWithProperties.SecondProperty)).Returns([second_property_first_metadata, second_property_second_metadata]);
    }
    ...
    ComplianceSchemaMetadata[] GetMetadata() => ((IEnumerable<ComplianceSchemaMetadata>)_context.Schema.ExtensionData[JsonSchemaGenerator.ComplianceKey]).ToArray();
