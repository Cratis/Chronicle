...
public class when_comparing_complex_nested_object_with_changes : given.an_object_comparer
{
    ...
    void Establish()
    {
        _left = new TopLevel("FortyTwo", 42, new("FortyThree", 43, new("FortyFour", 44)));
        _right = new TopLevel("FortyFive", 45, new("FortySix", 46, new("FortySeven", 47)));
    }
    ...
    [Fact] void should_have_first_difference_be_the_first_property_on_top_level() => _differences.ToArray()[0].PropertyPath.Path.ShouldEqual(nameof(TopLevel.StringValue));
    [Fact] void should_have_second_difference_be_the_second_property_on_top_level() => _differences.ToArray()[1].PropertyPath.Path.ShouldEqual(nameof(TopLevel.IntValue));
    ...
    [Fact] void should_have_third_difference_be_the_first_property_on_second_level() => _differences.ToArray()[2].PropertyPath.Path.ShouldEqual($"{nameof(TopLevel.Second)}.{nameof(SecondLevel.StringValue)}");
    [Fact] void should_have_forth_difference_be_the_second_property_on_second_level() => _differences.ToArray()[3].PropertyPath.Path.ShouldEqual($"{nameof(TopLevel.Second)}.{nameof(SecondLevel.IntValue)}");
    ...
    [Fact] void should_have_fifth_difference_be_the_first_property_on_third_level() => _differences.ToArray()[4].PropertyPath.Path.ShouldEqual($"{nameof(TopLevel.Second)}.{nameof(SecondLevel.Third)}.{nameof(ThirdLevel.StringValue)}");
    [Fact] void should_have_sixth_difference_be_the_second_property_on_third_level() => _differences.ToArray()[5].PropertyPath.Path.ShouldEqual($"{nameof(TopLevel.Second)}.{nameof(SecondLevel.Third)}.{nameof(ThirdLevel.IntValue)}");
