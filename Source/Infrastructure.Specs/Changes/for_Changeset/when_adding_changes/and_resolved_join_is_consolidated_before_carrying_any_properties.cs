...
/// <summary>
/// Characterizes the current behavior: a <see cref="ResolvedJoin"/> whose first change carries no
/// property differences is consolidated away before the joined values arrive, so the whole join is
/// silently dropped. The first assertion pins that data loss and is expected to change.
/// </summary>
public class and_resolved_join_is_consolidated_before_carrying_any_properties : given.a_changeset_with_parent
{
    ...
    [Fact] void should_lose_the_property_change() => ((IDictionary<string, object?>)_child)["name"].ShouldEqual("Original Name");
