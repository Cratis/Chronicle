...
public class when_creating_complex_path_with_nested_properties_and_array_indexers : Specification
{
    ...
    [Fact] void should_have_first_level_value_be_lower_cased_property_name() => _result.Segments.First().Value.ShouldEqual(FirstSegment.ToCamelCase());
    [Fact] void should_have_second_level_be_array_index() => _result.Segments.ToArray()[1].ShouldBeOfExactType<ArrayProperty>();
    [Fact] void should_have_second_level_value_be_lower_cased_property_name() => _result.Segments.ToArray()[1].Value.ShouldEqual(SecondSegment.ToCamelCase());
    [Fact] void should_have_third_level_be_property_name() => _result.Segments.ToArray()[2].ShouldBeOfExactType<PropertyName>();
    [Fact] void should_have_third_level_value_be_lower_cased_property_name() => _result.Segments.ToArray()[2].Value.ShouldEqual(ThirdSegment.ToCamelCase());
    [Fact] void should_have_forth_level_be_property_name() => _result.Segments.ToArray()[3].ShouldBeOfExactType<PropertyName>();
    [Fact] void should_have_forth_level_value_be_lower_cased_property_name() => _result.Segments.ToArray()[3].Value.ShouldEqual(ForthSegment.ToCamelCase());
    [Fact] void should_have_fifth_level_be_property_name() => _result.Segments.ToArray()[4].ShouldBeOfExactType<ArrayProperty>();
    [Fact] void should_have_fifth_level_value_be_lower_cased_property_name() => _result.Segments.ToArray()[4].Value.ShouldEqual(FifthSegment.ToCamelCase());
    [Fact] void should_have_sixth_level_be_property_name() => _result.Segments.ToArray()[5].ShouldBeOfExactType<PropertyName>();
    [Fact] void should_have_sixth_level_value_be_lower_cased_property_name() => _result.Segments.ToArray()[5].Value.ShouldEqual(SixthSegment.ToCamelCase());
}
