...
/// zero member, so <c language="csharp">0</c> is a value its own schema permits. Nothing but the nullability reading keeps it out.
/// <para>
/// The reading in question is the type-array one. An enum carries no <c language="csharp">format</c>, so the trailing <c language="csharp">?</c> the
/// converter used to test for is not where its nullability lives - it lives in <c language="csharp">"type": ["integer", "null"]</c>.
/// Drop that leg and this property, alone among the fixture's, starts materializing a <c language="csharp">0</c> that reads like a
/// deliberate <c language="csharp">NotSet</c> answer on a read model that never answered.
/// </para>
/// </summary>
