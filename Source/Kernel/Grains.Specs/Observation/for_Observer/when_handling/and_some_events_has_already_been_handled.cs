...
public class and_some_events_has_already_been_handled : given.an_observer_with_subscription_for_specific_event_type
{
    ...
    void Establish()
    {
        ...
        subscriber.Setup(_ => _.OnNext(Arg.Any<IEnumerable<AppendedEvent>>(), Arg.Any<ObserverSubscriberContext>())).Returns(Task.FromResult(ObserverSubscriberResult.Ok(43UL)));
    }

    async Task Because() => await observer.Handle("Something", events);

    [Fact] void should_forward_only_one_to_subscriber() => subscriber.Verify(_ => _.OnNext(Arg.Any<IEnumerable<AppendedEvent>>(), Arg.Any<ObserverSubscriberContext>()), Once());
    [Fact] void should_forward_last_event_to_subscriber() => subscriber.Verify(_ => _.OnNext(new[] { events.Last() }, Arg.Any<ObserverSubscriberContext>()), Once());
