...
public class when_seeding_already_seeded_events : given.an_event_seeding_grain
{
    ...
    void Establish()
    {
        ...
        var seededEntry = new Storage.Seeding.SeededEventEntry("event-source-1", "test-event-type", "{\"value\":\"test1\"}", false, "default");
        _state.State.ByEventType["test-event-type"] = [seededEntry];

        _entries = [
            new SeedingEntry("event-source-1", "test-event-type", "{\"value\":\"test1\"}", false, "default")
        ];
    }
