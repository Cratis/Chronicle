...
static class EventSequenceMutationRowConversion
{
    ...
    {
        var definition = ReconstructDefinition(row, target, row.ActiveMutationId!);
    ...
    /// <summary>
    /// Reconstructs a mutation definition from an active head row.
    /// </summary>
    ...
    internal static EventSequenceMutationDefinition ReconstructDefinition(
    ...
    internal static bool MatchesBoundRequest(EventSequenceMutationHeadEntry row, EventSequenceMutationRequest? request) =>
        request is { TargetSequence: not null, Origin.Sequence: not null, Origin.SequenceNumber: not null, Command.Payload: not null, Command.Hash: not null } &&
        string.Equals(row.EventSequenceId.Value, request.TargetSequence.Display, StringComparison.Ordinal) &&
