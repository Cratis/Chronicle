...
/// Retries validate the token binding and durable receipt without depending on residual head columns.
/// Archive inserts the receipt before releasing the exact terminal head; a retry safely finishes that
/// release after a crash, without clearing a newer mutation.
/// </para>
/// </remarks>
    ...
    IDatabase database) : IEventSequenceMutationRegistry
{
    ...
    {
        var previousOrdinal = new EventSequenceMutationOrdinal(active.Ordinal.Value - 1);
        var affected = await db.EventSequenceMutationHeads
            .Where(h => h.EventSequenceId == key && h.ActiveMutationId == null && h.LastAssignedOrdinal == previousOrdinal)
            .ExecuteUpdateAsync(
                s => s
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requestValidation = EventSequenceMutationValidator.ValidateRequest(request);
        if (!requestValidation.IsValid)
        {
            var exists = await db.EventSequenceMutationHistory.AnyAsync(h => h.MutationId == request.Id, cancellationToken) ||
                await db.EventSequenceMutationHeads.AnyAsync(h => h.ActiveMutationId == request.Id, cancellationToken);
            return exists
                ? EventSequenceMutationBeginResult.DefinitionConflict(request.Id)
                : EventSequenceMutationBeginResult.Invalid(requestValidation);
        }
        ...
        {
            var result = ResumeArchived(mutationScope, request, archivedRow);
            if (result.IsSuccess)
            {
                await ReleaseArchivedHead(db, key, EventSequenceMutationRowConversion.ToStorageHistory(archivedRow), cancellationToken);
            }

            return result;
        }

        var registeredHead = await db.EventSequenceMutationHeads.AsNoTracking()
            .FirstOrDefaultAsync(h => h.ActiveMutationId == request.Id, cancellationToken);
        if (registeredHead is not null)
        {
            return ResumeBound(mutationScope, request, registeredHead);
        }
        ...
        // Close the history-read/head-read window before attempting the ordinal-fenced claim.
        archivedRow = await db.EventSequenceMutationHistory.AsNoTracking()
            .FirstOrDefaultAsync(h => h.MutationId == request.Id, cancellationToken);
        if (archivedRow is not null)
        {
        ...
        if (headRow is not null && (!Enum.IsDefined(headRow.Coverage) || headRow.LastAssignedOrdinal is not { Value: >= 0 }))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var archived = await db.EventSequenceMutationHistory.AsNoTracking()
            .FirstOrDefaultAsync(h => h.MutationId == token.Id, cancellationToken);
        if (archived is not null)
        {
            return EventSequenceMutationRegistryTransitionResult.FromReceipt(mutationScope, token, EventSequenceMutationRowConversion.ToStorageHistory(archived));
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var affected = await db.EventSequenceMutationHeads
            .Where(h => h.EventSequenceId == key && h.ActiveMutationId == current.Id &&
                h.LastAssignedOrdinal == current.Ordinal && h.ActiveOrdinal == current.Ordinal &&
                h.ActiveDefinitionDigestV1 == current.Definition.DefinitionDigestV1 &&
                h.ActiveStateVersion == current.StateVersion && h.ActivePhase == current.Phase &&
                h.ActiveBlockedFrom == current.BlockedFrom && h.ActiveRepairState == current.RepairState)
            .ExecuteUpdateAsync(
                s => s
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (archived is not null)
        {
            return await RecoverArchive(db, mutationScope, token, archived, cancellationToken);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        catch (DbUpdateException)
        {
            ...
            await ReleaseArchivedHead(db, key, history, cancellationToken);
    ...
    async Task<EventSequenceMutationRegistryArchiveResult> RecoverArchive(
        ...
        EventSequenceKey scope,
        EventSequenceMutationStateToken token,
        EventSequenceMutationHistoryEntry row,
        CancellationToken cancellationToken)
    {
        var history = EventSequenceMutationRowConversion.ToStorageHistory(row);
        var result = EventSequenceMutationRegistryArchiveResult.FromReceipt(scope, token, history);
        if (result.IsSuccess)
        {
            await ReleaseArchivedHead(db, scope.EventSequenceId, history, cancellationToken);
        }

        return result;
    }

    async Task ReleaseArchivedHead(
        NamespaceDbContext db,
        EventSequenceId key,
        Chronicle.Storage.EventSequences.Mutations.EventSequenceMutationHistoryEntry history,
        CancellationToken cancellationToken)
    {
        var version = new EventSequenceMutationStateVersion(history.TerminalWitness.FinalStateVersion.Value - 1);
        await db.EventSequenceMutationHeads
            .Where(h => h.EventSequenceId == key && h.ActiveMutationId == history.Id &&
                h.LastAssignedOrdinal == history.Ordinal && h.ActiveOrdinal == history.Ordinal &&
                h.ActiveDefinitionDigestV1 == history.TerminalWitness.DefinitionDigestV1 &&
                h.ActiveStateVersion == version && h.ActivePhase == EventSequenceMutationPhase.SourceCommitted &&
                h.ActiveBlockedFrom == EventSequenceMutationPhase.None && h.ActiveRepairState == history.RepairState)
            .ExecuteUpdateAsync(
                s => s
                    .SetProperty(h => h.ActiveMutationId, (EventSequenceMutationId?)null)
                    .SetProperty(h => h.ActiveCommandPayload, (string?)null),
                cancellationToken);
    }
