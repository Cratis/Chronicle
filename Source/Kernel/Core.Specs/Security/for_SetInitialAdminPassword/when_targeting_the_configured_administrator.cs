...
public class when_targeting_the_configured_administrator : Specification
{
    ...
    void Establish()
    {
        ...
        _users.GetByUsername("chronicle-root").Returns(new StoredUser { Id = _administratorId, Username = "chronicle-root" });
        _eventLog = Substitute.For<IEventSequence>();
        _eventSerializer = Substitute.For<IEventSerializer>();
        _eventSerializer.Serialize(Arg.Any<object>()).Returns(call =>
        {
            var changed = call.Arg<UserPasswordChanged>();
            _users.GetById(Arg.Any<Concepts.Security.UserId>()).Returns(new StoredUser
            {
                Id = _administratorId,
                Username = "chronicle-root",
                HasLoggedIn = true,
                PasswordHash = changed.PasswordHash
            });
    ...
    async Task Because() =>
        _exception = await Catch.Exception(() => _command.Handle(
            _grainFactory,
            _storage,
            Microsoft.Extensions.Options.Options.Create(new Configuration.ChronicleOptions
            {
                Authentication = new() { AdminUser = new() { Username = "chronicle-root" } }
            }),
            _eventSerializer));
