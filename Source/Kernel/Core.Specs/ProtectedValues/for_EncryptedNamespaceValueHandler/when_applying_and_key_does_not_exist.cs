...
public class when_applying_and_key_does_not_exist : given.a_property_handler
{
    ...
    void Establish()
    {
        _keyStore.TryGetFor(EventStore, EventStoreNamespace, KeyIdentifier).Returns(Task.FromResult<EncryptionKey?>(null));
        ...
        _keyStore.GetOrAddFor(EventStore, EventStoreNamespace, KeyIdentifier, Arg.Any<EncryptionKey>()).Returns(Task.FromResult(_generatedKey));
    ...
    async Task Because() => _result = await _handler.Apply(EventStore, EventStoreNamespace, "some-subject", _input);

    [Fact] void should_generate_a_new_key() => _encryption.Received(1).GenerateKey();
    [Fact] async Task should_provision_the_key_under_the_namespace_wide_identity() =>
        await _keyStore.Received(1).GetOrAddFor(EventStore, EventStoreNamespace, KeyIdentifier, _generatedKey);
