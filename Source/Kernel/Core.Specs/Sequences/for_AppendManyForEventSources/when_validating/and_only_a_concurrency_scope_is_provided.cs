...
public class and_only_a_concurrency_scope_is_provided : Specification
{
    ...
    async Task Because() => _result = await _scenario.Validate(new AppendManyForEventSources(
        ...
        "event-log",
        [],
        ConcurrencyScopes: [new EventSourceConcurrencyScope("source", new ConcurrencyScope(0, true))]));
