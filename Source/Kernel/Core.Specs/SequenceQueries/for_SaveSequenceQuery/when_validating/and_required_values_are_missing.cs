...
public class and_required_values_are_missing : Specification
{
    ...
    async Task Because() => _result = await _scenario.Validate(new SaveSequenceQuery(string.Empty, string.Empty, string.Empty, SequenceQueryScope.User, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, null!, null!, null, null, string.Empty, false));
