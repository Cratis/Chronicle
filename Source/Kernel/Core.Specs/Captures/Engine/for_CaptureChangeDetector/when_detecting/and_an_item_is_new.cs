...
public class and_an_item_is_new : Specification
{
    ...
    [Fact] void should_be_an_added_change() => _result.First().Type.ShouldEqual(CaptureChangeType.Added);
    [Fact] void should_have_the_key() => _result.First().Key.ShouldEqual("42");
    [Fact] void should_not_have_a_previous_item() => _result.First().Previous.ShouldBeNull();
    [Fact] void should_have_the_current_item() => _result.First().Current.ShouldNotBeNull();
}
