...
{
    readonly CommandScenario<StopCapture> _scenario = ChronicleCommandScenario.For<StopCapture>();
    ...
    async Task Because() => _result = await _scenario.Validate(new StopCapture("some-event-store", Guid.NewGuid()));
