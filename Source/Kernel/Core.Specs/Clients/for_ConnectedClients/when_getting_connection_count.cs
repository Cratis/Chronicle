...
public class when_getting_connection_count : Specification
{
    ...
    async Task Because()
    {
        await _connectedClients.OnClientConnected(ConnectionId.New(), "1.0.0", false);
        await _connectedClients.OnClientConnected(ConnectionId.New(), "1.0.0", false);
        _count = await _connectedClients.GetConnectionCount();
    }

    [Fact] void should_return_the_number_of_connected_clients() => _count.ShouldEqual(2);
}
