...
public class and_new_event_types_are_added_with_other_changes : given.a_projection_replay_recommendation_evaluator
{
    EventType[] _result = [];
    ...
    void Establish()
    {
        ...
        var current = CreateDefinition(
            from: new Dictionary<EventType, FromDefinition>
            {
                [EventType1] = new(new Dictionary<PropertyPath, string> { ["name"] = "$event.updatedName" }, "$eventSourceId", null),
    ...
    [Fact] void should_return_no_added_event_types() => _result.ShouldBeEmpty();
}
