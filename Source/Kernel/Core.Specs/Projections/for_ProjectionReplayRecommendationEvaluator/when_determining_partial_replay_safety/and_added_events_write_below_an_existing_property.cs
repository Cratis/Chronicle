...
public class and_added_events_write_below_an_existing_property : given.a_projection_replay_recommendation_evaluator
{
    bool _result;
    ...
    void Because()
    {
        ...
        {
            [EventType1] = new(new Dictionary<PropertyPath, string> { ["address"] = "address" }, PropertyExpression.NotSet, null)
        ...
        var current = CreateDefinition(from: new Dictionary<EventType, FromDefinition>
        {
            ...
            [EventType2] = new(new Dictionary<PropertyPath, string> { ["address.city"] = "city" }, PropertyExpression.NotSet, null)
        }) with { AutoMap = AutoMap.Disabled };
