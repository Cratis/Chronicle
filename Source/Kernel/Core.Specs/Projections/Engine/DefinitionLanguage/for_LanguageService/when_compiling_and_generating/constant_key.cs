...
public class constant_key : given.a_language_service_with_schemas<given.Users>
{
    const string Declaration = """
        projection MyProjection => Users
          from UserRegistered
            key literal "some-constant-key"
            name = name
        """;
    ...
    [Fact] void should_have_from_user_registered() => _result.From.ContainsKey((EventType)"UserRegistered").ShouldBeTrue();
    [Fact] void should_have_constant_key_expression() => _result.From[(EventType)"UserRegistered"].Key.Value.ShouldEqual("$value(some-constant-key)");
