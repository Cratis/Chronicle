...
public class and_all_values_are_provided : Specification
{
    ...
    async Task Because() => _result = await _scenario.Validate(new RegisterEventTypes("some-event-store", [new EventTypeRegistration { Type = new EventType { Id = "SomeEvent" }, Schema = "{}" }], false));
