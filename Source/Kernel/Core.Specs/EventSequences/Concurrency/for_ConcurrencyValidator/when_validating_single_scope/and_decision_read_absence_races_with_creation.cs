...
public class and_decision_read_absence_races_with_creation : given.a_concurrency_validator
{
    ...
    void Establish()
    {
        _scope = new ConcurrencyScope(EventSequenceNumber.BeforeFirst, true, null, null, null, _types);
        _eventSequenceStorage.GetTailSequenceNumber(_types, _source, null, null, null).Returns((EventSequenceNumber)1);
    }

    async Task Because() => _result = await _validator.Validate(_source, _scope);

    [Fact] void should_reject_the_absence_race() => _result.HasValue.ShouldBeTrue();
