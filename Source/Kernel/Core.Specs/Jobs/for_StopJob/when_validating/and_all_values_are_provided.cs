...
{
    readonly CommandScenario<StopJob> _scenario = new();
    ...
    async Task Because() => _result = await _scenario.Validate(new StopJob("some-event-store", "some-namespace", _jobId.Value));
