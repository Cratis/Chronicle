...
public class the_manager : Specification
{
    ...
    async Task Establish()
    {
        ...
        _jobStorage.GetJob(Arg.Any<JobId>()).Returns(callInfo => Task.FromResult(
            _storedJobs.SingleOrDefault(job => job.Id == callInfo.Arg<JobId>()) ?? Catch.Failed<JobState, Cratis.Orleans.Storage.Jobs.JobError>(Cratis.Orleans.Storage.Jobs.JobError.NotFound)));
