...
public class and_an_exception_is_thrown_with_no_prior_handled_events : given.a_performing_job_step
{
    void Establish() =>
        _observerSubscriber
            .OnNext(Arg.Any<Key>(), Arg.Any<IEnumerable<AppendedEvent>>(), Arg.Any<ObserverSubscriberContext>())
            .Returns(Task.FromException<ObserverSubscriberResult>(new Exception("subscriber threw unexpectedly")));
    ...
    [Fact] void should_report_it_as_the_subscriber_failing_to_handle_the_events() =>
        _observer.Received(1).PartitionFailed(
            Arg.Any<Key>(),
