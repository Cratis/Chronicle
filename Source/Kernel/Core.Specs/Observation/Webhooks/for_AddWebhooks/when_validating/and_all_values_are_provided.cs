...
public class and_all_values_are_provided : Specification
{
    ...
    async Task Because() => _result = await _scenario.Validate(new AddWebhooks("some-event-store", [new WebhookDefinition { Identifier = "some-webhook", EventSequenceId = "event-log" }]));
