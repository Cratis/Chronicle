...
public sealed class EventSequenceMutationRegistryTransitionResult
{
    ...
    /// <summary>
    /// Reports a verified terminal receipt without reconstructing an archived command payload.
    /// </summary>
    ...
    /// <returns>The archived result, or a state conflict.</returns>
    public static EventSequenceMutationRegistryTransitionResult FromReceipt(
        EventSequenceKey scope,
        EventSequenceMutationStateToken token,
        EventSequenceMutationHistoryEntry history) =>
        EventSequenceMutationReceipt.Matches(scope, token, history)
            ? new(EventSequenceMutationRegistryTransitionOutcome.AlreadyArchived, null, null, history, EventSequenceMutationRegistryError.Unknown, null)
            : StateConflict();
