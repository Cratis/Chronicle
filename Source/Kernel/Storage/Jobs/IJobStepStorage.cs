...
public interface IJobStepStorage
{
    ...
    Task<Catch> RemoveAllForJob(JobId jobId);
    ...
    /// <param name="jobId"><see cref="JobId"/> the step belongs to.</param>
    /// <returns>Awaitable task.</returns>
    Task<Catch> RemoveAllNonFailedForJob(JobId jobId);
    ...
    /// <returns>Awaitable task.</returns>
    Task<Catch> Remove(JobId jobId, JobStepId jobStepId);
    ...
    Task<Catch<IImmutableList<JobStepState>>> GetForJob(JobId jobId, params JobStepStatus[] statuses);
    ...
    Task<Catch<int>> CountForJob(JobId jobId, params JobStepStatus[] statuses);
    ...
    Catch<IObservable<IEnumerable<JobStepState>>> ObserveForJob(JobId jobId);
    ...
    Task<Catch<TJobStepState, JobStepError>> Read<TJobStepState>(JobId jobId, JobStepId jobStepId);
    ...
    /// <returns>Awaitable task.</returns>
    Task<Catch<None, JobStepError>> Save<TJobStepState>(JobId jobId, JobStepId jobStepId, TJobStepState state);
    ...
    /// <returns>Awaitable task.</returns>
    Task<Catch<None, JobStepError>> MoveToFailed<TJobStepState>(JobId jobId, JobStepId jobStepId, TJobStepState jobStepState);
}
