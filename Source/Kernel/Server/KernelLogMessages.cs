...
internal static partial class KernelLogMessages
{
    ...
    [LoggerMessage(LogLevel.Information, "Configuring server to listen on management port {ManagementPort} (HTTP/1.1) and gRPC port {GrpcPort} (HTTP/2)")]
    internal static partial void ServerListening(this ILogger<Kernel> logger, int managementPort, int grpcPort);

    [LoggerMessage(LogLevel.Information, "Cratis Chronicle Server configured successfully - starting services")]
