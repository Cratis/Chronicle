...
public class when_resolving_expression_with_two_properties_mapped : given.a_resolver
{
    ...
    async Task Because() => _result = await _resolver.Resolve(_projection, $"{WellKnownExpressions.Composite}({FirstProperty}=$first, {SecondProperty}=$second)", "target")(null!, null!, null!);
