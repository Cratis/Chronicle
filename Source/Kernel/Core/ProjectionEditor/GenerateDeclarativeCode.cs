...
[Command]
public record GenerateDeclarativeCode(EventStoreName EventStore, EventStoreNamespaceName Namespace, string Declaration, DraftReadModel? DraftReadModel = null)
{
    /// <summary>
    ...
    public async Task<GeneratedCodeResult> Handle(IProjections projections)
    {
        ...
        return result.Value switch
        {
            ...
            ProjectionDeclarationParsingErrors errors => new GeneratedCodeResult(
                string.Empty,
                errors.Errors.ToApi()),
            _ => throw new UnexpectedProjectionResult(nameof(GenerateDeclarativeCode))
        };
    }
}
