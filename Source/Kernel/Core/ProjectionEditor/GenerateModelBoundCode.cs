...
/// <summary>
/// Represents a request to generate model-bound read model client code from projection declaration language.
/// </summary>
...
{
    /// <summary>
    /// Handles the command by compiling the declaration and generating the read model as client code.
    /// </summary>
    ...
    public Task<GeneratedCodeResult> Handle(IStorage storage, ILanguageService languageService) =>
        ProjectionCodeGeneration.Generate(
            ...
            ProjectionCodeStyle.ModelBound,
            storage,
            languageService);
}
