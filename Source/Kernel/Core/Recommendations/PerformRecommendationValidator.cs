// Copyright (c) Cratis. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for full license information.

using Cratis.Arc.Commands;
using FluentValidation;

namespace Cratis.Chronicle.Recommendations;

/// <summary>
/// Represents the validator for <see cref="PerformRecommendation"/>.
/// </summary>
internal class PerformRecommendationValidator : CommandValidator<PerformRecommendation>
{
    /// <summary>
    /// Initializes a new instance of the <see cref="PerformRecommendationValidator"/> class.
    /// </summary>
    public PerformRecommendationValidator()
    {
        RuleFor(_ => _.EventStore).NotEmpty().WithMessage("Event store name is required.");
        RuleFor(_ => _.Namespace).NotEmpty().WithMessage("Namespace name is required.");
        RuleFor(_ => _.RecommendationId).NotEmpty().WithMessage("Recommendation identifier is required.");
    }
}
