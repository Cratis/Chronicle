...
public class when_getting_the_same_event_store_concurrently : given.a_storage
{
    ...
    void Establish() => _clusterStorage.RendezvousOnGetEventStores = true;

    async Task Because()
    {
        var first = Task.Run(() => _storage.GetEventStore(_eventStore));
        _firstCallerArrived = _clusterStorage.WaitForFirstCaller();
    ...
    [Fact] void should_have_released_both_callers() => _clusterStorage.CallersWereReleased.ShouldBeTrue();
    [Fact] void should_have_created_two_candidate_instances() => _clusterStorage.CreatedStorages.Count().ShouldEqual(2);
