...
public class and_a_property_change_targets_the_same_collection_as_a_child_added : given.a_changeset_converter
{
    ...
    void Establish()
    {
        ...
        var childAdded = new ChildAdded(childState, _rolesProperty, new PropertyPath("Role"), "Administrator", ArrayIndexers.NoIndexers);
        var propertyDifference = new PropertyDifference(_rolesProperty, Array.Empty<object>(), new[] { childState });
    ...
    [Fact] void should_not_convert_the_collection_property_change_to_a_set() => _mongoDBConverter.DidNotReceive().ToBsonValue(Arg.Any<object?>(), Arg.Is<PropertyPath>(_ => _ == _rolesProperty));
