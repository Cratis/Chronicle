// Copyright (c) Cratis. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for full license information.

// <auto-generated />

#nullable enable

using Cratis.Chronicle.Contracts.Commands;
using Cratis.Chronicle.Contracts.Primitives;
using Cratis.Chronicle.Contracts.Queries;
using ProtoBuf;
using ProtoBuf.Grpc;
using ProtoBuf.Grpc.Configuration;

namespace Cratis.Chronicle.Contracts.Jobs;
/// <summary>
/// Defines the gRPC service contract for Jobs.
/// </summary>
[Service]
public interface IJobs
{
    /// <summary>
    /// Executes the DeleteJob command.
    /// </summary>
    /// <param name = "request">The DeleteJob request.</param>
    /// <param name = "callContext">The gRPC call context.</param>
    /// <returns>The command result.</returns>
    [Operation]
    Task<CommandResult> DeleteJob(DeleteJobRequest request, CallContext callContext = default);
    /// <summary>
    /// Executes the ResumeJob command.
    /// </summary>
    /// <param name = "request">The ResumeJob request.</param>
    /// <param name = "callContext">The gRPC call context.</param>
    /// <returns>The command result.</returns>
    [Operation]
    Task<CommandResult> ResumeJob(ResumeJobRequest request, CallContext callContext = default);
    /// <summary>
    /// Executes the StopJob command.
    /// </summary>
    /// <param name = "request">The StopJob request.</param>
    /// <param name = "callContext">The gRPC call context.</param>
    /// <returns>The command result.</returns>
    [Operation]
    Task<CommandResult> StopJob(StopJobRequest request, CallContext callContext = default);
    /// <summary>
    /// Executes the GetJobSteps query.
    /// </summary>
    /// <param name = "request">The query request parameters.</param>
    /// <param name = "callContext">The gRPC call context.</param>
    /// <returns>The query result.</returns>
    [Operation]
    Task<QueryResult<IEnumerable<JobStepSummaryResponse>>> GetJobSteps(GetJobStepsRequest request, CallContext callContext = default);
    /// <summary>
    /// Executes the AllJobs query.
    /// </summary>
    /// <param name = "request">The query request parameters.</param>
    /// <param name = "callContext">The gRPC call context.</param>
    /// <returns>The query result.</returns>
    [Operation]
    Task<QueryResult<IEnumerable<JobSummaryResponse>>> AllJobs(AllJobsRequest request, CallContext callContext = default);
    /// <summary>
    /// Executes the ObserveJobs query.
    /// </summary>
    /// <param name = "request">The query request parameters.</param>
    /// <param name = "callContext">The gRPC call context.</param>
    /// <returns>The query result.</returns>
    [Operation]
    IObservable<QueryResult<IEnumerable<JobSummaryResponse>>> ObserveJobs(ObserveJobsRequest request, CallContext callContext = default);
}

/// <summary>
/// Represents the DeleteJobRequest message.
/// </summary>
[ProtoContract]
public class DeleteJobRequest
{
    /// <summary>
    /// Gets or sets the EventStore.
    /// </summary>
    [ProtoMember(1)]
    public string EventStore { get; set; }

    /// <summary>
    /// Gets or sets the Namespace.
    /// </summary>
    [ProtoMember(2)]
    public string Namespace { get; set; }

    /// <summary>
    /// Gets or sets the JobId.
    /// </summary>
    [ProtoMember(3)]
    public Guid JobId { get; set; }
}

/// <summary>
/// Represents the ResumeJobRequest message.
/// </summary>
[ProtoContract]
public class ResumeJobRequest
{
    /// <summary>
    /// Gets or sets the EventStore.
    /// </summary>
    [ProtoMember(1)]
    public string EventStore { get; set; }

    /// <summary>
    /// Gets or sets the Namespace.
    /// </summary>
    [ProtoMember(2)]
    public string Namespace { get; set; }

    /// <summary>
    /// Gets or sets the JobId.
    /// </summary>
    [ProtoMember(3)]
    public Guid JobId { get; set; }
}

/// <summary>
/// Represents the StopJobRequest message.
/// </summary>
[ProtoContract]
public class StopJobRequest
{
    /// <summary>
    /// Gets or sets the EventStore.
    /// </summary>
    [ProtoMember(1)]
    public string EventStore { get; set; }

    /// <summary>
    /// Gets or sets the Namespace.
    /// </summary>
    [ProtoMember(2)]
    public string Namespace { get; set; }

    /// <summary>
    /// Gets or sets the JobId.
    /// </summary>
    [ProtoMember(3)]
    public Guid JobId { get; set; }
}

/// <summary>
/// Represents the JobStepSummaryResponse message.
/// </summary>
[ProtoContract]
public class JobStepSummaryResponse
{
    /// <summary>
    /// Gets or sets the Id.
    /// </summary>
    [ProtoMember(1)]
    public Guid Id { get; set; }

    /// <summary>
    /// Gets or sets the Type.
    /// </summary>
    [ProtoMember(2)]
    public string Type { get; set; }

    /// <summary>
    /// Gets or sets the Name.
    /// </summary>
    [ProtoMember(3)]
    public string Name { get; set; }

    /// <summary>
    /// Gets or sets the Status.
    /// </summary>
    [ProtoMember(4)]
    public global::Cratis.Chronicle.Contracts.Jobs.JobStepStatus Status { get; set; }

    /// <summary>
    /// Gets or sets the StatusChanges.
    /// </summary>
    [ProtoMember(5)]
    public IEnumerable<global::Cratis.Chronicle.Contracts.Jobs.JobStepStatusChanged> StatusChanges { get; set; } = new List<global::Cratis.Chronicle.Contracts.Jobs.JobStepStatusChanged>();

    /// <summary>
    /// Gets or sets the Progress.
    /// </summary>
    [ProtoMember(6)]
    public global::Cratis.Chronicle.Contracts.Jobs.JobStepProgress Progress { get; set; }
}

/// <summary>
/// Represents the GetJobStepsRequest message.
/// </summary>
[ProtoContract]
public class GetJobStepsRequest
{
    /// <summary>
    /// Gets or sets the eventStore.
    /// </summary>
    [ProtoMember(1)]
    public string EventStore { get; set; }

    /// <summary>
    /// Gets or sets the namespace.
    /// </summary>
    [ProtoMember(2)]
    public string Namespace { get; set; }

    /// <summary>
    /// Gets or sets the jobId.
    /// </summary>
    [ProtoMember(3)]
    public Guid JobId { get; set; }
}

/// <summary>
/// Represents the JobSummaryResponse message.
/// </summary>
[ProtoContract]
public class JobSummaryResponse
{
    /// <summary>
    /// Gets or sets the Id.
    /// </summary>
    [ProtoMember(1)]
    public Guid Id { get; set; }

    /// <summary>
    /// Gets or sets the Details.
    /// </summary>
    [ProtoMember(2)]
    public string Details { get; set; }

    /// <summary>
    /// Gets or sets the Type.
    /// </summary>
    [ProtoMember(3)]
    public string Type { get; set; }

    /// <summary>
    /// Gets or sets the Status.
    /// </summary>
    [ProtoMember(4)]
    public global::Cratis.Chronicle.Contracts.Jobs.JobStatus Status { get; set; }

    /// <summary>
    /// Gets or sets the Created.
    /// </summary>
    [ProtoMember(5)]
    public global::Cratis.Chronicle.Contracts.Primitives.SerializableDateTimeOffset Created { get; set; }

    /// <summary>
    /// Gets or sets the StatusChanges.
    /// </summary>
    [ProtoMember(6)]
    public IEnumerable<global::Cratis.Chronicle.Contracts.Jobs.JobStatusChanged> StatusChanges { get; set; } = new List<global::Cratis.Chronicle.Contracts.Jobs.JobStatusChanged>();

    /// <summary>
    /// Gets or sets the Progress.
    /// </summary>
    [ProtoMember(7)]
    public global::Cratis.Chronicle.Contracts.Jobs.JobProgress Progress { get; set; }
}

/// <summary>
/// Represents the AllJobsRequest message.
/// </summary>
[ProtoContract]
public class AllJobsRequest
{
    /// <summary>
    /// Gets or sets the eventStore.
    /// </summary>
    [ProtoMember(1)]
    public string EventStore { get; set; }

    /// <summary>
    /// Gets or sets the namespace.
    /// </summary>
    [ProtoMember(2)]
    public string Namespace { get; set; }
}

/// <summary>
/// Represents the ObserveJobsRequest message.
/// </summary>
[ProtoContract]
public class ObserveJobsRequest
{
    /// <summary>
    /// Gets or sets the eventStore.
    /// </summary>
    [ProtoMember(1)]
    public string EventStore { get; set; }

    /// <summary>
    /// Gets or sets the namespace.
    /// </summary>
    [ProtoMember(2)]
    public string Namespace { get; set; }
}