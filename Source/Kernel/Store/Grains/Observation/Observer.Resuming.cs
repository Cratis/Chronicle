...
public partial class Observer
{
    ...
    public async Task TryResumePartition(EventSourceId eventSourceId)
    {
        ...
        if (State.IsRecoveringPartition(failedPartition.EventSourceId) && _streamSubscription is not null)
        {
            return;
        }
        ...
        var tailSequenceNumber = await _eventSequenceStorageProvider.GetTailSequenceNumber(State.EventTypes, eventSourceId);
    ...
    async Task HandleEventForRecoveringPartitionedObserver(AppendedEvent @event, EventSequenceNumber tailSequenceNumber)
    {
        ...
        else
        {
            ...
            {
                var actualTailSequenceNumber = await _eventSequenceStorageProvider.GetTailSequenceNumber(State.EventTypes, @event.Context.EventSourceId);
