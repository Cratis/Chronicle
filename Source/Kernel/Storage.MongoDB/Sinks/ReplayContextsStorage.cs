...
public class ReplayContextsStorage(IEventStoreNamespaceDatabase database) : IReplayContextsStorage
{
    ...
    public Task Save(Chronicle.Storage.Sinks.ReplayContext context)
    {
        ...
        return _collection.ReplaceOneAsync(_ => _.Identifier == storageContext.Identifier, storageContext, new ReplaceOptions { IsUpsert = true });
    }

    /// <inheritdoc/>
    public Task<Result<Chronicle.Storage.Sinks.ReplayContext, GetContextError>> TryGet(ReadModelIdentifier readModel)
    {
        var filter = Builders<ReplayContext>.Filter.Eq(_ => _.Identifier, readModel);
