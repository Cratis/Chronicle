...
{
    static readonly Regex _regularExpression = new("\\$subtract\\((?<expression>[A-Za-z.]*)\\)", RegexOptions.Compiled | RegexOptions.ExplicitCapture, TimeSpan.FromSeconds(1));
    ...
    public PropertyMapper<AppendedEvent, ExpandoObject> Resolve(PropertyPath targetProperty, string expression)
    {
        ...
        return PropertyMappers.SubtractWithEventValueProvider(targetProperty, _eventValueProviderExpressionResolvers.Resolve(match.Groups["expression"].Value));
    }
}
