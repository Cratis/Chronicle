...
public class when_archiving_a_terminal_mutation : given.a_mutation_registry
{
    ...
    int _historiesById;
    int _historiesByTargetAndOrdinal;
    ...
    async Task Because()
    {
        ...
        _permanentConflict = await _registry.Begin(
            _request with { Command = new("changed", "changed-hash") },
            _proposedTarget);
        ...
        _historiesById = Count("_historiesById");
        _historiesByTargetAndOrdinal = Count("_historiesByTargetAndOrdinal");
    }
    ...
    [Fact] void should_index_history_by_id() => _historiesById.ShouldEqual(1);
    [Fact] void should_index_history_by_target_and_ordinal() => _historiesByTargetAndOrdinal.ShouldEqual(1);

    int Count(string field) =>
        ((IDictionary)typeof(EventSequenceMutationRegistryState)
            .GetField(field, BindingFlags.Instance | BindingFlags.NonPublic)!
            .GetValue(_state)!).Count;
}
