...
public class when_redacting_an_event_source : given.a_storage_with_appended_events
{
    ...
    async Task Establish()
    {
        ...
        ((IDictionary<string, object?>)content)["secret"] = "revised-payload";
        await _storage.Revise(0, _eventType, CorrelationId.New(), [], [], DateTimeOffset.UtcNow, content, new EventHash("original-payload-hash"));
    }
    ...
    [Fact] void should_clear_revisions_on_the_revised_event() => _storage.Events.Single(_ => _.Context.SequenceNumber == (EventSequenceNumber)0UL).Revisions.ShouldBeEmpty();
    [Fact] void should_clear_the_revised_content_hash() => _storage.Events.Single(_ => _.Context.SequenceNumber == (EventSequenceNumber)0UL).Context.Hash.ShouldEqual(EventHash.NotSet);
}
