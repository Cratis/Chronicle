    ...
    ILogger<Reactor> logger) : Grain<ReactorDefinition>, IReactor
{
    ...
    public async Task SetDefinitionAndSubscribe(ReactorDefinition definition)
    {
        ...
        if (!_subscribed)
        {
            ...
            var subscriberArgs = new ReactorObserverSubscriptionArgs(connectedClient, definition.IsReplayable);
            await _observer.Subscribe<IReactorObserverSubscriber>(ObserverType.Reactor, eventTypes, localSiloDetails.SiloAddress, subscriberArgs);
            _subscribed = true;
        }
    }
