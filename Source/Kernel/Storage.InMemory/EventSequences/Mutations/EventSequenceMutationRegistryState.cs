...
sealed class EventSequenceMutationRegistryState
{
        ...
        EventSequenceMutationTarget proposedTarget)
    {
        ...
        lock (_gate)
        {
            if (_historiesById.TryGetValue(request.Id, out var archivedHistory))
            {
                var archivedValidation = EventSequenceMutationValidator.ValidateRequest(request);
                if (!archivedValidation.IsValid)
                {
                    return EventSequenceMutationBeginResult.DefinitionConflict(request.Id);
                }

                var archivedScope = Scope(request.TargetSequence, eventStore, @namespace);
                var archivedDefinition = EventSequenceMutationDefinition.Create(archivedScope, request, archivedHistory.Target);
                if (archivedDefinition.DefinitionDigestV1 != archivedHistory.TerminalWitness.DefinitionDigestV1)
                {
                    return EventSequenceMutationBeginResult.DefinitionConflict(request.Id);
                }

                var archived = new EventSequenceMutationRegistration(archivedDefinition, EventSequenceMutationRegistryLifecycle.Archived, archivedHistory.Ordinal, archivedHistory.TerminalWitness);
                return EventSequenceMutationBeginResult.Archived(archivedScope, archived, archivedHistory);
            }
        ...
        EventSequenceMutationTransition transition)
    {
        ...
        lock (_gate)
        {
            if (_historiesById.TryGetValue(token.Id, out var archivedHistory))
            {
                return EventSequenceMutationRegistryTransitionResult.FromReceipt(Scope(target, eventStore, @namespace), token, archivedHistory);
            }
        ...
        EventSequenceMutationStateToken token)
    {
        ...
        lock (_gate)
        {
            if (_historiesById.TryGetValue(token.Id, out var receipt))
            {
                return EventSequenceMutationRegistryArchiveResult.FromReceipt(Scope(target, eventStore, @namespace), token, receipt);
            }
            ...
            _registrations.Remove(token.Id);
