...
/// (for example via <c language="csharp">[SetFrom]</c>).
/// <para>
/// Both forms apply wherever the attribute is written: on the root read model, on a <c language="csharp">[ChildrenFrom]</c> child
/// record and on a <c language="csharp">[Nested]</c> type. Exclusions are scoped to the model that declares them and are matched by
