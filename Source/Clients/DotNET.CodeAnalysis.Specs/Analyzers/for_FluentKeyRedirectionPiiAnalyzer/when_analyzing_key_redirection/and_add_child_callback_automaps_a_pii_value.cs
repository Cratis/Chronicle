...
public class and_add_child_callback_automaps_a_pii_value : given.a_fluent_key_redirection_pii_analyzer
{
    const string Usage = """
    ...
    void Because() => _result = AnalyzerVerifier<CodeAnalysis.Analyzers.FluentKeyRedirectionPiiAnalyzer>.VerifyAnalyzer(
        CreateSource(Usage),
        new ExpectedDiagnostic(DiagnosticIds.KeyRedirectionPii, DiagnosticSeverity.Warning, "Contacts", "ContactAdded", "EmailAddress", "RequestId", "Id"));

    [Fact] Task should_report_the_automapped_child_value_reaching_the_document() => _result;
}
