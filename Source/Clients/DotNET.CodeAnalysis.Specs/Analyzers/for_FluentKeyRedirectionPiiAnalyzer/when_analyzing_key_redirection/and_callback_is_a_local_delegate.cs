...
public class and_callback_is_a_local_delegate : given.a_fluent_key_redirection_pii_analyzer
{
    const string Usage = """
    ...
    public class RequestSummaryProjection : IProjectionFor<RequestSummary>
    {
        ...
        {
            Action<IFromBuilder<RequestSummary, AdvisorNamed>> configure = from => from.{|#0:UsingKey|}(e => e.RequestId);
            builder.From<AdvisorNamed>(configure);
        }
    }
    """;
    ...
    [Fact] Task should_analyze_the_delegate_initializer() => _result;
}
