...
public class and_an_opaque_instance_method_result_cannot_suppress_automap : given.a_fluent_key_redirection_pii_analyzer
{
    const string Usage = """
    ...
    {
        public IProjectionBuilderFor<RequestSummary> Enable(IProjectionBuilderFor<RequestSummary> builder)
        {
            var result = builder;
            return result;
        }
    ...
    {
        readonly Helpers _helpers = new();

        public void Define(IProjectionBuilderFor<RequestSummary> builder) => _helpers.Enable(builder)
