...
public class and_an_opaque_method_result_cannot_suppress_automap : given.a_fluent_key_redirection_pii_analyzer
{
    const string Usage = """
    ...
    {
        public void Define(IProjectionBuilderFor<RequestSummary> builder) => Forward(builder)
            .From<AdvisorNamed>(from => from.{|#0:UsingKey|}(e => e.RequestId));

        static IProjectionBuilderFor<RequestSummary> Forward(IProjectionBuilderFor<RequestSummary> builder)
        {
            var result = builder;
            return result;
        }
