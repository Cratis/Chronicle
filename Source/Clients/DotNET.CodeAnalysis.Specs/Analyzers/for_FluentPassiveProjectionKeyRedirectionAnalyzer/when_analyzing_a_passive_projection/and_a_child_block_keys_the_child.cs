// Copyright (c) Cratis. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for full license information.

using Cratis.Chronicle.CodeAnalysis.Specs.Testing;

namespace Cratis.Chronicle.CodeAnalysis.Specs.Analyzers.for_FluentPassiveProjectionKeyRedirectionAnalyzer.when_analyzing_a_passive_projection;

/// <summary>
/// UsingKey inside a Children block identifies the child within its collection. The document a passive read
/// resolves is still the parent's, keyed by the event source the replay already walks.
/// </summary>
public class and_a_child_block_keys_the_child : given.a_fluent_passive_projection_key_redirection_analyzer
{
    const string Usage = """
    public record LineAdded(string Sku, int Quantity);

    public record OrderLine(string Sku, int Quantity);

    [Passive]
    public record Order(
        [Key] string Id,
        IEnumerable<OrderLine> Lines);

    public class OrderProjection : IProjectionFor<Order>
    {
        public void Define(IProjectionBuilderFor<Order> builder) => builder
            .Children(m => m.Lines, c => c
                .From<LineAdded>(_ => _
                    .UsingKey(e => e.Sku)
                    .Set(m => m.Quantity).To(e => e.Quantity)));
    }
    """;

    Task _result;

    void Because() => _result = AnalyzerVerifier<CodeAnalysis.Analyzers.FluentPassiveProjectionKeyRedirectionAnalyzer>.VerifyAnalyzer(CreateSource(Usage));

    [Fact] Task should_not_report_any_diagnostic() => _result;
}
