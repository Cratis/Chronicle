...
public class and_colliding_event_is_aggregate_only : given.an_auto_map_same_name_property_collision_analyzer
{
    const string Usage = """
    public record Opened(Guid Id, string Name);
    public record Other(Guid Id, string Name);

    [FromEvent<Opened>]
    public record Account(
        Guid Id,
        [SetFrom<Opened>(nameof(Opened.Name))] string Name,
        [Count<Other>] int Total);
    """;
