...
public class and_projection_injects_event_log : given.a_projection_side_effect_analyzer
{
    const string Usage = """
    ...
    {
        public MyProjection({|#0:Cratis.Chronicle.EventSequences.IEventLog eventLog|}) { }
    ...
    void Because() => _result = AnalyzerVerifier<CodeAnalysis.Analyzers.ProjectionSideEffectAnalyzer>.VerifyAnalyzer(
        CreateSource(Usage),
        new ExpectedDiagnostic(DiagnosticIds.ProjectionMustNotHaveSideEffects, DiagnosticSeverity.Error, "MyProjection", "IEventLog"));
