...
public class and_expression_lambda_contains_conditional : given.a_projection_expression_lambda_analyzer
{
    const string Usage = """
    public class AnEvent { public bool Active { get; set; } public string Name { get; set; } public string Fallback { get; set; } }
    ...
    public class MyProjection : Cratis.Chronicle.Projections.IProjectionFor<ReadModel>
    {
        public void Define(Cratis.Chronicle.Projections.IProjectionBuilderFor<ReadModel> builder) =>
            builder.From<AnEvent>(evt => evt
                .Set(x => x.Name)
                .To(e => {|#0:e.Active ? e.Name : e.Fallback|}));
    }
    """;
    ...
    [Fact] Task should_report_conditional_diagnostic() => _result;
}
