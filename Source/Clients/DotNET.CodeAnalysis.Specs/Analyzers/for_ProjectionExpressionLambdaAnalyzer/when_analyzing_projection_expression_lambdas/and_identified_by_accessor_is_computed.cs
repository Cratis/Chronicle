...
public class and_identified_by_accessor_is_computed : given.a_projection_expression_lambda_analyzer
{
    const string Usage = """
    public class AnEvent { public string Reference { get; set; } }
    public class ReadModel { public string Id { get; set; } }
    ...
    public class MyProjection : Cratis.Chronicle.Projections.IProjectionFor<ReadModel>
    {
        public void Define(Cratis.Chronicle.Projections.IProjectionBuilderFor<ReadModel> builder) =>
            builder.From<AnEvent>(evt => evt
                .IdentifiedBy(x => {|#0:x.Id.ToUpperInvariant()|}));
    }
    """;
    ...
    [Fact] Task should_report_identified_by_diagnostic() => _result;
}
