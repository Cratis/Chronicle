...
public class and_expression_lambda_ignores_parameter : given.a_projection_expression_lambda_analyzer
{
    const string Usage = """
    public class AnEvent { public string Name { get; set; } }
    public class ReadModel { public System.DateTimeOffset When { get; set; } }
    ...
    public class MyProjection : Cratis.Chronicle.Projections.IProjectionFor<ReadModel>
    {
        public void Define(Cratis.Chronicle.Projections.IProjectionBuilderFor<ReadModel> builder) =>
            builder.From<AnEvent>(evt => evt
                .Set(x => x.When)
                .To(e => {|#0:System.DateTimeOffset.UtcNow|}));
    }
    """;
    ...
    [Fact] Task should_report_parameter_ignoring_diagnostic() => _result;
}
