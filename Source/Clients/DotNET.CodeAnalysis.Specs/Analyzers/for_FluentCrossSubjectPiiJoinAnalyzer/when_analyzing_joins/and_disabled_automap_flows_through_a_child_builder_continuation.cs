...
public class and_disabled_automap_flows_through_a_child_builder_continuation : given.a_fluent_cross_subject_pii_join_analyzer
{
    const string Usage = """
    public record AdvisorNamed([PII] string FullName);
    public record AdvisorAdded(AdvisorSummary Advisor);
    ...
    public class AdvisorBookProjection : IProjectionFor<AdvisorBook>
    {
        public void Define(IProjectionBuilderFor<AdvisorBook> builder) => builder
            .Children(m => m.Advisors, children => children
                .NoAutoMap()
                .FromEventProperty<AdvisorAdded>(e => e.Advisor)
                .Join<AdvisorNamed>());
    }
    """;
