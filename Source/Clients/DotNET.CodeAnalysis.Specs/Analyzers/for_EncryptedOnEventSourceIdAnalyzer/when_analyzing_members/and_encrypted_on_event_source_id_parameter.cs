...
public class and_encrypted_on_event_source_id_parameter : given.an_encrypted_on_event_source_id_analyzer
{
    const string Usage = """
    public record PartnerId(Guid Value) : EventSourceId<Guid>(Value);

    public record PartnerIntegrationConfigured(
        {|#0:[Encrypted]|} PartnerId PartnerId,
        string ApiKey);
    """;
    ...
    void Because() => _result = AnalyzerVerifier<CodeAnalysis.Analyzers.EncryptedOnEventSourceIdAnalyzer>.VerifyAnalyzer(
        CreateSource(Usage),
        new ExpectedDiagnostic(DiagnosticIds.EncryptedOnEventSourceId, DiagnosticSeverity.Error, "PartnerId"));
