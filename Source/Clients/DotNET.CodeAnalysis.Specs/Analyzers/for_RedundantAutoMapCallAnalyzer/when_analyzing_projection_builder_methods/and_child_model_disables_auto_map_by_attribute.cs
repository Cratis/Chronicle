...
public class and_child_model_disables_auto_map_by_attribute : given.a_redundant_auto_map_call_analyzer
{
    const string Usage = """
    [Cratis.Chronicle.Projections.NoAutoMap]
    ...
    public class MyProjection : Cratis.Chronicle.Projections.IProjectionFor<ReadModel>
    {
        public void Define(Cratis.Chronicle.Projections.IProjectionBuilderFor<ReadModel> builder) =>
            builder.Children(m => m.Children, child => child.AutoMap());
    }
    """;
    ...
    [Fact] Task should_not_report_the_child_call() => _result;
}
