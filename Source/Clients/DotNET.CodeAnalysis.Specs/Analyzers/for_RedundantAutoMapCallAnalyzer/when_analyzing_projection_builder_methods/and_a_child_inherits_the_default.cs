...
public class and_a_child_inherits_the_default : given.a_redundant_auto_map_call_analyzer
{
    const string Usage = """
    ...
    public class MyProjection : Cratis.Chronicle.Projections.IProjectionFor<ReadModel>
    {
        public void Define(Cratis.Chronicle.Projections.IProjectionBuilderFor<ReadModel> builder) =>
            builder.Children(m => m.Children, child => child{|#0:.AutoMap()|});
    }
    """;
    ...
    void Because() => _result = AnalyzerVerifier<CodeAnalysis.Analyzers.RedundantAutoMapCallAnalyzer>.VerifyAnalyzer(
        CreateSource(Usage),
        new ExpectedDiagnostic(DiagnosticIds.RedundantAutoMapCall, Microsoft.CodeAnalysis.DiagnosticSeverity.Warning));

    [Fact] Task should_report_redundant_auto_map_diagnostic() => _result;
}
