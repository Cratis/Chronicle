...
public class and_the_state_depends_on_a_branch : given.a_redundant_auto_map_call_analyzer
{
    const string Usage = """
    ...
    public class MyProjection : Cratis.Chronicle.Projections.IProjectionFor<ReadModel>
    {
        ...
        {
            builder.NoAutoMap();
            if (DateTime.UtcNow.Ticks > 0) builder.AutoMap();
            builder.AutoMap();
        }
    }
    """;
    ...
    [Fact] Task should_not_report_a_call_with_unknown_state() => _result;
}
