...
namespace Cratis.Chronicle.Connections.for_CertificateLoader.given;
...
public class a_certificate_file : Specification
{
    ...
    void Establish()
    {
        _certificatePath = Path.Combine(Path.GetTempPath(), $"chronicle-client-certificate-{Guid.NewGuid():N}.pfx");
        using var key = RSA.Create(2048);
        var request = new CertificateRequest("CN=chronicle-client-specs", key, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
