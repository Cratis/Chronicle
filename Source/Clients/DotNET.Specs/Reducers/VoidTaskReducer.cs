...
public class VoidTaskReducer
{
    ...
    public Task Reduce(ValidEvent @event, ReadModel? initial, EventContext context)
    {
        _receivedEventsAndContexts.Add(new(@event, context));
        _readModels.Add(initial);
        return Task.CompletedTask; // Void task indicates deletion
    }
}