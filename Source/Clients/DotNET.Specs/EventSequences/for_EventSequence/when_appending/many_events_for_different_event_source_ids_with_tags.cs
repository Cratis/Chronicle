...
public class many_events_for_different_event_source_ids_with_tags : given.an_event_sequence
{
    ...
    async Task Because() => await _eventSequence.AppendMany(_events, tags: _tags);

    [Fact] void should_append_all_events_with_tags() => _command.Events.All(e => e.Tags.SequenceEqual(_tags)).ShouldBeTrue();
}
