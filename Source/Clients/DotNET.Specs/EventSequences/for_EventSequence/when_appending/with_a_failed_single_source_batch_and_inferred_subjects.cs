...
public class with_a_failed_single_source_batch_and_inferred_subjects : given.an_acknowledged_append
{
    void Establish()
    {
    ...
    async Task Because() => await _eventSequence.AppendMany(_source, [new EventWithSubject("first-subject"), new EventWithSubject("second-subject")]);

    [Fact] void should_notify_each_attempt() => _notifications.Length.ShouldEqual(2);
    [Fact] void should_preserve_the_subjects_sent_in_the_request() => _notifications.Select(_ => _.Event.Context.Subject.Value).ShouldEqual(_legacyRequest.Events.Select(_ => _.Subject));
