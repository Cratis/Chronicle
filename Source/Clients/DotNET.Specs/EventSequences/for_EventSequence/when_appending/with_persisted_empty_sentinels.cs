...
public class with_persisted_empty_sentinels : given.an_acknowledged_append
{
    ...
    void Establish()
    {
        ...
        // An empty subject on the wire only comes from a server that predates carrying one, where it meant the
        // event source is the subject. A read resolves it the same way, so an append and a read agree on that.
