...
public class and_it_has_events : given.an_event_sequence
{
    ...
    void Establish()
    {
        ...
        _sequences.HasEventsForEventSourceId(Arg.Is<Contracts.Sequences.HasEventsForEventSourceIdRequest>(
            req => req.EventSourceId == _eventSourceId)).Returns(Task.FromResult(QueryResult<Contracts.Sequences.EventSourceEventsResponse>.Success(Guid.NewGuid(), new() { HasEvents = true })));
    }

    async Task Because() => _result = await _eventSequence.HasEventsFor(_eventSourceId);

    [Fact] void should_return_true_when_events_exist() => _result.ShouldBeTrue();
}
