...
public class with_event_only : given.a_resolver
{
    object?[] _arguments;

    async Task Because() => _arguments = await _resolver.Resolve(
        MethodNamed(nameof(ReactorWithDependencies.EventOnly)),
    ...
    [Fact] void should_have_a_single_argument() => _arguments.Length.ShouldEqual(1);
