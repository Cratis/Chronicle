...
public class and_aggregate_root_has_handle_methods : given.a_stateless_aggregate_root_mutator
{
    ...
    [Fact]
    void should_handle_events() => _eventHandlers.Received().Handle(_aggregateRoot, Arg.Is<IEnumerable<EventAndContext>>(arg => arg.Select(_ => _.Event).SequenceEqual(_events)));
}
