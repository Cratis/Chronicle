...
public class and_accessor_boxes_a_value_type : given.an_event
{
    bool _result;
    ...
    void Because()
    {
        Expression<Func<SomeEvent, object>> accessor = e => e.Age;
        _result = accessor.TryGetPropertyPath(out _path);
    }

    [Fact] void should_succeed() => _result.ShouldBeTrue();
    [Fact] void should_strip_the_conversion_and_extract_the_property_path() => _path.Path.ShouldEqual("Age");
}
