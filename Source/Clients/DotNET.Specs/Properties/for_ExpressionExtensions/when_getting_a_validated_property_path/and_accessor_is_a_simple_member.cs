...
public class and_accessor_is_a_simple_member : given.an_event
{
    bool _result;
    ...
    void Because()
    {
        Expression<Func<SomeEvent, string>> accessor = e => e.Name;
        _result = accessor.TryGetPropertyPath(out _path);
    }

    [Fact] void should_succeed() => _result.ShouldBeTrue();
    [Fact] void should_extract_the_property_path() => _path.Path.ShouldEqual("Name");
}
