...
public class and_accessor_is_a_method_call : Specification
{
    ...
    void Because()
    {
        ...
        _error = Catch.Exception(() => builder.IdentifiedBy(c => c.Id.ToUpperInvariant()));
    }

    [Fact] void should_throw_invalid_property_expression() => _error.ShouldBeOfExactType<InvalidPropertyExpression>();
    [Fact] void should_name_the_child_read_model() => _error!.Message.ShouldContain(typeof(Child).FullName!);
}
