...
public class with_grandchildren_having_property_level_removed_with : given.a_model_bound_projection_builder
{
    ...
    void Because()
    {
        _result = builder.Build(typeof(RootWithNestedRemovableGrandchildren));
        _result.Children.TryGetValue(nameof(RootWithNestedRemovableGrandchildren.Items), out _itemsDef);
        _itemsDef?.Children.TryGetValue(nameof(NestedRemovableItem.SubItems), out _subItemsDef);
    }
    ...
    [Fact] void should_use_specified_parent_key_on_removed_with() => _subItemsDef.RemovedWith.Values.First().ParentKey.ShouldEqual(naming_policy.GetPropertyName(new Properties.PropertyPath(nameof(SubItemRemovedFromItem.ItemId))));
}
