...
public class with_a_read_model_type : given.a_discovered_projection
{
    ...
    async Task Because() => _result = await _projections.GetFailedPartitionsFor(typeof(TheReadModel));

    [Fact] void should_ask_for_the_failed_partitions_of_the_projection() => _failedPartitions.Received(1).GetFailedPartitionsFor(_projections.GetProjectionIdForModel<TheReadModel>().Value);
