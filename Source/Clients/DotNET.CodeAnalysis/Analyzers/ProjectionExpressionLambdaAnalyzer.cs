...
/// <summary>
/// Analyzer that checks expression lambdas passed to projection builder methods only contain member access.
/// </summary>
[DiagnosticAnalyzer(LanguageNames.CSharp)]
public class ProjectionExpressionLambdaAnalyzer : DiagnosticAnalyzer
{
    static readonly DiagnosticDescriptor Rule = new(
        id: DiagnosticIds.ProjectionExpressionLambdaMustOnlyAccessMembers,
        title: "Projection expression lambda must only access members",
        messageFormat: "Projection '{0}' passes an expression lambda with non-member-access code '{1}'. Expression lambdas in the projection builder are used to extract property paths at startup—they are never executed at runtime.",
        category: "Usage",
    ...
    static void AnalyzeLambda(SyntaxNodeAnalysisContext context)
    {
        ...
        // Check if this lambda is inside the Define() method of a class implementing IProjectionFor<T>
        ...
        if (!WellKnownTypes.ImplementsIProjectionFor(methodSymbol.ContainingType, context.Compilation))
        {
            return;
        }
