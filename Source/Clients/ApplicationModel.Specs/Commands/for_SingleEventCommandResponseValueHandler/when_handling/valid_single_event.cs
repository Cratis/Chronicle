...
public class valid_single_event : given.a_single_event_command_response_value_handler
{
    ...
    [Fact] void should_append_event_to_event_log() => _eventLog.Received(1).Append(Arg.Any<EventSourceId>(), _event);
    [Fact] void should_append_with_correct_event_source_id() => _eventLog.Received().Append(((TestCommand)_commandContext.Command).EventSourceId, _event);
