...
public class single_event_collection : given.an_events_command_response_value_handler
{
    ...
    void Establish()
    {
        _testEvent = new TestEvent("Single Event");
        _events = [_testEvent];
    }
    ...
    [Fact] void should_append_events_to_event_log() => _eventLog.Received(1).Append(Arg.Any<EventSourceId>(), _events);
    [Fact] void should_append_with_correct_event_source_id() => _eventLog.Received().Append(((TestCommand)_commandContext.Command).EventSourceId, _events);
