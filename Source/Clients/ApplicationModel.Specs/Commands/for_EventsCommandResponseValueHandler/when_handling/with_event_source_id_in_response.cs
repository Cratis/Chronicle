...
public class with_event_source_id_in_response : given.an_events_command_response_value_handler
{
    ...
    [Fact] void should_append_events_to_event_log() => _eventLog.Received(1).AppendMany(_eventSourceId, _events, Arg.Any<EventStreamType?>(), Arg.Any<EventStreamId?>(), Arg.Any<EventSourceType?>());
    [Fact] void should_append_with_event_source_id_from_response() => _eventLog.Received().AppendMany(_eventSourceId, _events, Arg.Any<EventStreamType?>(), Arg.Any<EventStreamId?>(), Arg.Any<EventSourceType?>());
