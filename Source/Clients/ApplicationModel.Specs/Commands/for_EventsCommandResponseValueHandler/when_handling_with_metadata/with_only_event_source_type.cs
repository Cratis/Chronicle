...
public class with_only_event_source_type : given.an_events_command_response_value_handler
{
    ...
    void Establish()
    {
        _events = [new TestEvent("Event with source type")];
    ...
    [Fact] void should_append_with_event_source_type() => _eventLog.Received().AppendMany(Arg.Any<EventSourceId>(), Arg.Any<IEnumerable<object>>(), null, null, _eventSourceType);
}
