...
{
    /// <inheritdoc/>
    public Task<TAggregateRoot> Get<TAggregateRoot>(EventSourceId id, EventStreamId? streamId = default, EventSource? eventSource = default)
        where TAggregateRoot : IAggregateRoot
    {
        var key = new AggregateRootKey(eventSource ?? EventSource.Default, id, streamId ?? EventStreamId.Default);
