...
public class when_an_append_many_with_errors_is_observed : given.an_event_append_collection
{
    ...
    void Because()
    {
        var result = AppendManyResult.Failed(CorrelationId.New(), [_error]);
    ...
    [Fact] void should_include_the_error() => _collection.All[0].Errors.Count().ShouldEqual(1);
    [Fact] void should_have_the_correct_error() => _collection.All[0].Errors.Single().ShouldEqual(_error);
    [Fact] void should_have_errors() => _collection.All[0].HasErrors.ShouldBeTrue();
