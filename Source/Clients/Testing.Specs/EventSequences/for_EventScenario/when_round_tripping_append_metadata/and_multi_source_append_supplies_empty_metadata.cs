...
public class and_multi_source_append_supplies_empty_metadata : Specification, IDisposable
{
    ...
    async Task Because()
    {
        _result = await _scenario.EventLog.AppendMany([new EventForEventSourceId(_source, new TestEvent("first")) { EventSourceType = EventSourceType.Unspecified, EventStreamType = new EventStreamType(string.Empty), EventStreamId = EventStreamId.NotSet }]);
