...
public class and_single_append_supplies_empty_metadata : Specification, IDisposable
{
    ...
    async Task Because()
    {
        _result = await _scenario.EventLog.Append(_source, new TestEvent("first"), eventStreamType: new EventStreamType(string.Empty), eventStreamId: EventStreamId.NotSet, eventSourceType: EventSourceType.Unspecified);
