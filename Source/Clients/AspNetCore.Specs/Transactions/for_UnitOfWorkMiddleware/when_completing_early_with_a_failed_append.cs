...
public class when_completing_early_with_a_failed_append : Specification
{
    ...
    [Fact] void should_report_the_protected_read_conflict() => _completed.GetDecisionConflicts().ShouldContain(new DecisionConflict(typeof(object), "source"));
    [Fact] void should_send_the_decision_scope() => _sequence.Received(1).AppendMany(
        Arg.Any<IEnumerable<EventForEventSourceId>>(),
        Arg.Any<CorrelationId?>(),
        Arg.Any<IEnumerable<string>>(),
        Arg.Is<IDictionary<EventSourceId, ConcurrencyScope>>(_ => _.ContainsKey("source") && _["source"].SequenceNumber == 4));
