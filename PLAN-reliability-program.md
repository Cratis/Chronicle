...
- **Proto generation is still broken on `main`, and silently** — the Aug 15 exit-code fix + regen was
  reverted within hours (`d5ae2df47`, `3c65086cb`). Confirmed by running it on 2026-08-19: it fails
  **22 of 23 packages and exits 0**, every one on the same run-scoped reservation check for
  `UniqueEventTypeConstraintDefinition`. Only `events_constraints.proto` regenerates; the other 22
  checked-in schemas survive because a failed package leaves the old file in place, so nothing looks
  wrong. **#3734 fixes it** (verified: 23/23, zero errors, clean diff) and is held for Einar. Until it
  lands, do not build anything that assumes regeneration works — see the 8.a note in §8.
...
|---|---|---|---|
| **8.a** | **Regenerate-and-diff gate** for generated output | 133 tracked `// @generated by Cratis` files and 23 committed `.proto` files; no `git diff --exit-code` exists anywhere in CI, and every CI build passes `-p:DisableProxyGenerator=true`, so CI has never produced a proxy to compare. Catches hand-edits *and* staleness with one predicate | A job regenerates protos + proxies and fails on any diff; **no determinism-stabilization pass is needed — both halves already reproduce byte-identically, measured** |
...
**8.a — measured, and smaller than it looked (2026-08-19).** The audit's inputs were both off, and
the correction changes the plan rather than just the numbers.

- **133 tracked generated files, not 154** — and every one of them is under `Source/Workbench/Api`,
  produced by the Arc proxy generator from `Source/Clients/Api`. The 1,822 files matching
  `@generated by Cratis` on disk are overwhelmingly build output; only the 133 are committed. The
  gate therefore has exactly one proxy producer to run, not a survey of the tree.
- **23 committed protos, not 6.**
- **Neither half needs the anticipated "determinism-stabilization pass."** Both were run and diffed:
  `dotnet build Source/Clients/Api -c Debug` reproduces all 133 proxies byte-identically (empty
  `git status`), and the proto generator reproduces all 23 schemas byte-identically. The gate can be
  a plain `git diff --exit-code` from the start.

The real blocker is sequencing, and it is the more interesting finding. **On `main` today the proto
generator fails 22 of its 23 packages and still exits 0** — every package but `events_constraints`
dies on the same run-scoped reservation check for `UniqueEventTypeConstraintDefinition`. The
checked-in schemas are only intact because the failing packages leave the previous files untouched,
which is precisely the shape of the silent-success incident §6 records as still churning. A
regenerate-and-diff gate built on `main` as it stands would compare against 22 schemas that were
never regenerated and pass — **vacuously green, the exact failure this program exists to stop.**

**#3734 is the fix**, and it was verified rather than assumed: on that branch the same command
generates 23 of 23 with zero errors and a clean `git diff`. It also already carries a protoc
validation step, which is a real part of 8.a's intent. So 8.a's proto half is mostly built and
waiting on a branch held for Einar; the sequencing is **#3734 first, then the diff gate**, and
building the gate before it would be actively harmful.

One exclusion is a contract, not an oversight: **gRPC contract generation must stay out of the
gate.** `Source/Kernel/Core/Core.csproj` gates `GenerateGrpcContracts` off deliberately, because the
Observation artifacts in Core cannot yet reproduce the hand-written
`Contracts/Observation/IObservers.cs` — regenerating would silently drop `ClearObserverQuarantine`,
`GetObserverInformation`, `GetConnectedClientsForObserver`, `WaitForCompletion` and
