...
- Never include Copilot prompt content in the PR description. Remove any "Original prompt" / coding agent transcript blocks before publishing. This includes an agent tool's own footer — a `Generated with [Claude Code]` line or a `Co-Authored-By: Claude` trailer is transcript residue like any other, and 24 of the last 400 releases published one.

**This section is machine-checked.** `.github/workflows/pull-request-body.yml` runs `assert-pull-request-body.py` on every pull request, including on `edited`, because the body changes without a push. It fails on a heading outside the sanctioned set, a sanctioned section left empty, an issue-closing keyword, and agent-transcript residue.

The heading rule is an **allowlist, never a denylist** — `Summary`, `Added`, `Changed`, `Fixed`, `Removed`, `Deprecated`, `Security`. A denylist only ever learns the last mistake: banning `## Test plan` would have left `## Root cause`, `## Verified`, `## Suggested regression test` and `## Files affected` to ship, and all four did. Internal detail is not forbidden — it is just not a *release note*, so put it in a pull request comment, where reviewers read it and consumers do not.

Widening the allowlist is a deliberate decision, not a way to make a red check green. If a change genuinely needs a section the list lacks, say in the pull request why a consumer reading the releases page wants it.
