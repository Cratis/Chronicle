...
description: >
  ...
  label a PR with a proper description, merge it, close the issues it resolves,
...
branch → commits → PR → merge → close issues → cleanup workflow.
...
- **Related issue** — optional GitHub issue number; if unknown, search first. Referenced issues that the change fully resolves are closed after the merge
